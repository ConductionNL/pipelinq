...
| 3 · Social publishing | Account connection through the broker, adapters in review-cost order (Mastodon and Bluesky, LinkedIn member then page, X with a spend cap, Meta), composer and calendar with approvals, advocacy share flow, daily metrics pull, read-only inbox | `social-accounts-and-connect`, `social-post-composer-and-calendar`, `social-adapter-fediverse`, `social-adapter-linkedin`, `social-adapter-x`, `social-adapter-meta`, `social-advocacy-share-flow`, `social-metrics-pull` | A post drafted from an article is approved and published to a company page, a colleague's profile and Mastodon at the scheduled time, and shows views the next morning |
