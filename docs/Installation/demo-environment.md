...
This page gets a working Pipelinq running on your own machine in two commands. You end with pipelines that move and transform registry data.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/pipelinq/development/pipelinq-compose.yaml
```
...
==> installing pipelinq <version>
==> apps present: integriq openregister pipelinq thematiq
```
...
```bash
curl -s http://localhost:8610/status.php
```
...
| What | Where |
| --- | --- |
| **Pipelinq** | [http://localhost:8610/apps/pipelinq/](http://localhost:8610/apps/pipelinq/) |
| Admin interface | [http://localhost:8610](http://localhost:8610) — `admin` / `admin` |
...
| `pipelinq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing pipelinq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8610/apps/pipelinq/"
...
curl -s -u admin:admin "http://localhost:8610/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f pipelinq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f pipelinq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f pipelinq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
