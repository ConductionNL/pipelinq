 ...
 *
 * CTI adapter for the CallVoip telephony platform (Dutch SME market leader).
 *
 * @category Service
 ...
 * CallVoip adapter.
 *
 * Inbound webhooks are authenticated with an HMAC-SHA256 signature over the
 * raw request body, supplied in the `X-Signature` header. Outbound origination
 * posts to `{apiBaseUrl}/calls`.
 *
 * @spec openspec/changes/cti-screenpop-adapter/tasks.md#task-1.2
 */
class CallVoipAdapter implements CtiAdapterInterface
{
    ...
    /**
     * Set the API base URL (used by the service after loading config).
     *
    ...
    /**
     * Verify the HMAC-SHA256 signature over the raw body (REQ-CTI-007).
     *
    ...
    /**
     * Normalise a CallVoip webhook payload (REQ-CTI-001/004/005/008).
     *
    ...
    public function handleInboundWebhook(array $payload): CtiWebhookResult
    {
        ...
        return new CtiWebhookResult(
            eventType: $eventType,
            externalCallId: $this->stringOrNull(value: ($payload['callId'] ?? null)),
            fromNumber: $this->stringOrNull(value: ($payload['from'] ?? null)),
            toNumber: $this->stringOrNull(value: ($payload['to'] ?? null)),
            extension: $this->stringOrNull(value: ($payload['extension'] ?? null)),
    ...
    /**
     * Map a raw CallVoip event name to a normalised event type.
     *
    ...
    private function normaliseEventType(string $event): string
    {
        ...
        if (in_array($event, $known, true) === true) {
            return $event;
        }
    ...
    /**
     * Originate an outbound call via the CallVoip REST API.
     *
    ...
    {
        if ($this->apiBaseUrl === '') {
            return new CtiCallResult(success: false, message: 'CallVoip API base URL is not configured.');
        }
        ...
        try {
            ...
            if (is_array($body) === true) {
                $callId = $this->stringOrNull(value: ($body['callId'] ?? null));
            }

            return new CtiCallResult(success: true, externalCallId: $callId, message: 'Call originated.');
        } catch (Throwable $e) {
            $this->logger->warning('CallVoip originate failed', ['exception' => $e->getMessage()]);
