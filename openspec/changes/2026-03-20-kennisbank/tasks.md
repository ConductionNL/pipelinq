...
- [ ] 0.1 Verify full-text search is covered by OpenRegister `_search` param — `KennisbankController` MUST NOT implement a custom search engine. Document in PR description: "search delegated to OpenRegister IndexService".
- [ ] 0.2 Verify CRUD for agents uses `objectStore` directly (no custom CRUD controller for kennisartikel/kenniscategorie). `KennisbankController` covers only the public read endpoint and feedback submission.
- [ ] 0.3 Confirm `NotificationService` is available in the existing app (check `lib/Service/`) — do NOT create a parallel notification service; reuse the existing one.
- [ ] 0.4 Search `openspec/specs/` for any existing knowledge base or article spec. If found, reference rather than duplicate.
- [ ] 0.5 Verify `marked` npm package is not already imported under a different name — check `package.json` for existing Markdown rendering libs before adding.

## 1. Data Model

- [ ] 1.1 Add `kennisartikel` schema to `lib/Settings/pipelinq_register.json` with all properties per design:
...
- [ ] 1.2 Add `kenniscategorie` schema to `pipelinq_register.json`:
  - Required: `name` (string)
  - Optional: `slug`, `parent` (uuid), `description`, `order` (integer, default 0), `icon`
- [ ] 1.3 Add `kennisfeedback` schema to `pipelinq_register.json`:
...
- [ ] 1.4 Update the register's `schemas` list in `pipelinq_register.json` to include `kennisartikel`, `kenniscategorie`, `kennisfeedback`

## 2. Seed Data

- [ ] 2.1 Add 5 kenniscategorie seed objects to `lib/Settings/pipelinq_register.json` under `components.objects[]` using `@self` envelope:
...
- [ ] 2.2 Add 4 kennisartikel seed objects (Dutch realistic content, mix of status/visibility):
...
- [ ] 2.3 Add 3 kennisfeedback seed objects:
...
- [ ] 2.4 Verify idempotency: re-importing `pipelinq_register.json` MUST NOT create duplicate objects (match by slug via existing `ImportHandler` logic)

## 3. Backend Service

- [ ] 3.1 Create `lib/Service/KennisbankService.php`
  - SPDX header: `// SPDX-License-Identifier: EUPL-1.2`
  - Constructor: inject `ObjectService`, `IAppConfig`, `IUserSession`, `LoggerInterface`
- [ ] 3.2 Implement `getPublicArticles(string $search, string $category, int $limit, int $offset): array`
...
- [ ] 3.3 Implement `getPublicArticle(string $id): array`
...
- [ ] 3.4 Implement `submitFeedback(string $articleId, string $rating, ?string $comment, string $agentUid): array`
...
- [ ] 3.5 Implement `recalculateScore(string $articleId): float`
...
- [ ] 3.6 All `catch (\Throwable $e)` blocks MUST log full exception via `$this->logger->error()` — NEVER expose `$e->getMessage()` to callers

## 4. Backend Controller

- [ ] 4.1 Create `lib/Controller/KennisbankController.php`
  - SPDX header: `// SPDX-License-Identifier: EUPL-1.2`
...
- [ ] 4.2 Implement `index(): JSONResponse`
...
- [ ] 4.3 Implement `show(string $id): JSONResponse`
  - Attribute: `#[PublicPage] #[NoCSRFRequired]`
...
- [ ] 4.4 Implement `feedback(): JSONResponse`
...
- [ ] 4.5 Verify NEVER calls `$e->getMessage()` in any response body

## 5. Routes

- [ ] 5.1 Add kennisbank API routes to `appinfo/routes.php`:
  ```php
...
- [ ] 5.2 Add OPTIONS pre-flight route for public endpoints if a CORS wildcard is configured

## 6. Frontend Views

- [ ] 6.1 Create `src/views/kennisbank/KennisbankHome.vue`
  - SPDX header: `<!-- SPDX-License-Identifier: EUPL-1.2 -->`
...
- [ ] 6.2 Create `src/views/kennisbank/ArticleList.vue`
  - SPDX header: `<!-- SPDX-License-Identifier: EUPL-1.2 -->`
...
- [ ] 6.3 Create `src/views/kennisbank/ArticleDetail.vue`
  - SPDX header: `<!-- SPDX-License-Identifier: EUPL-1.2 -->`
...
- [ ] 6.4 Create `src/views/kennisbank/ArticleEditor.vue`
  - SPDX header: `<!-- SPDX-License-Identifier: EUPL-1.2 -->`
...
- [ ] 6.5 Create `src/views/kennisbank/CategoryManager.vue`
  - SPDX header: `<!-- SPDX-License-Identifier: EUPL-1.2 -->`
...
- [ ] 7.1 Add kennisbank routes to `src/router/index.js`:
  ```js
...
- [ ] 7.2 Add "Kennisbank" `NcAppNavigationItem` to `src/navigation/MainMenu.vue`:
...
- [ ] 8.2 Add translations to `l10n/en.json` and `l10n/nl.json`:
...
- [ ] 9.1 Run `npm run build` — verify zero build errors
- [ ] 9.2 Run `composer check:strict` (or `php -l` on all new PHP files) — verify zero syntax errors
- [ ] 9.3 Smoke test `GET /api/kennisbank/public` without auth — verify 200 and `results` array shape; verify only `status=gepubliceerd` AND `visibility=openbaar` articles appear
- [ ] 9.4 Smoke test `GET /api/kennisbank/public/{id}` with an `intern` article UUID — verify 404
- [ ] 9.5 Smoke test `POST /api/kennisbank/feedback` without auth — verify 401
- [ ] 9.6 Smoke test `POST /api/kennisbank/feedback` with auth, missing `rating` — verify 400 with `message` field; no stack trace
- [ ] 9.7 Manual browser test: publish an article → verify it appears in KennisbankHome search; archive it → verify it disappears
- [ ] 9.8 Manual browser test: submit "Nuttig" feedback → verify `usefulnessScore` updates on article detail
- [ ] 9.9 Manual browser test: create subcategory → verify it appears nested under parent in category tree
- [ ] 9.10 Run SPDX header check — MUST return zero files without header:
  ```bash
  ...
  ```
- [ ] 9.11 Verify ObjectService calls use 3 positional args throughout KennisbankService:
  ```bash
  grep -n 'findObjects\|saveObject\|findObject' lib/Service/KennisbankService.php
  ```
  Every call MUST have exactly 3 arguments.
- [ ] 9.12 Verify no `$e->getMessage()` in controller response bodies:
  ```bash
  grep -n 'getMessage()' lib/Controller/KennisbankController.php
  ```
  MUST return zero matches.
- [ ] 9.13 Verify all Vue components import from `@conduction/nextcloud-vue` only — NEVER `@nextcloud/vue`:
  ```bash
  grep -rn "from '@nextcloud/vue'" src/views/kennisbank/
  ```
  MUST return zero matches.
- [ ] 9.14 Verify seed data import is idempotent: trigger `ImportHandler` twice; assert kenniscategorie count does not double.
