...
- Each rule notifies the handling `agent`, carries `originApp: "pipelinq"`, delivers on `["nc-notification", "web-push"]`, and exposes a primary "Open client" action targeting the related Client via the relation-resolved `object-detail` kind.
...
- **Relation resolution requires `client` to be populated.** `client` is optional on the Contactmoment. If absent, the engine cannot resolve the "Open client" deeplink. The action should degrade gracefully (engine responsibility); the rule still notifies the agent of the inbound interaction.
