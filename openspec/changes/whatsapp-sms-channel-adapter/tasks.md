...
- [ ] 0.1 Verify that `omnichannel-registratie` already provides `conversation` and `message` schemas and that we are only extending them, not redefining them
...
- [ ] 0.2 Verify that `client-management` provides the `contact` schema and placeholder contact creation is a simple API call
...
- [ ] 0.3 Verify that `openconnector` is the correct place for webhook receiver registration (not rebuilding signature verification)
...
- [ ] 0.4 Verify that `NotificationService` can fire admin notifications for template rejection, budget alerts, and failed SMS
...
- [ ] 1.1 Update `lib/Settings/pipelinq_register.json` to add the `channel_provider` schema
...
- [ ] 1.2 Update `lib/Settings/pipelinq_register.json` to add the `message_template` schema
  - **spec_ref**: Design § Data Layer
  - **files**: `lib/Settings/pipelinq_register.json`
...
- [ ] 1.3 Update `lib/Settings/pipelinq_register.json` to add the `consent_record` schema
  - **spec_ref**: Design § Data Layer
  - **files**: `lib/Settings/pipelinq_register.json`
...
- [ ] 1.4 Update `lib/Settings/pipelinq_register.json` to add the `message_send_budget` schema
  - **spec_ref**: Design § Data Layer
  - **files**: `lib/Settings/pipelinq_register.json`
...
- [ ] 1.5 Extend the `message` schema in `lib/Settings/pipelinq_register.json` with new fields
  - **spec_ref**: Design § Data Layer
  - **files**: `lib/Settings/pipelinq_register.json`
...
- [ ] 1.6 Register the four new schemas in `appinfo/register.json` or equivalent app-level schema manifest
...
- [ ] 2.1 Create `lib/Service/WhatsAppAdapter.php` with `send(contact, body, templateId?, parameters?)` method
...
- [ ] 2.2 Implement session-window tracking in WhatsAppAdapter (24-hour window from last inbound)
  - **spec_ref**: REQ-002
  - **files**: `lib/Service/WhatsAppAdapter.php`
...
- [ ] 2.3 Implement template parameter validation in WhatsAppAdapter
  - **spec_ref**: REQ-001
  - **files**: `lib/Service/WhatsAppAdapter.php`
...
- [ ] 2.4 Implement provider selection logic in WhatsAppAdapter (Meta Cloud API primary, BSP fallback)
...
- [ ] 2.5 Create `lib/Service/WhatsAppProviderClient.php` abstraction for Meta Cloud API calls
...
- [ ] 2.6 Implement inbound webhook handling in WhatsAppAdapter (REQ-003)
  - **spec_ref**: REQ-003
...
- [ ] 2.7 Implement media attachment handling (REQ-008)
  - **spec_ref**: REQ-008
...
- [ ] 2.8 Implement template approval sync (REQ-009)
  - **spec_ref**: REQ-009
...
- [ ] 3.1 Create `lib/Service/SmsAdapter.php` with `send(to, body, providerHint?)` method
  - **spec_ref**: REQ-004
...
- [ ] 3.2 Create `lib/Service/SmsProviderFactory.php` to instantiate provider clients (Twilio, MessageBird, CM.com)
  - **spec_ref**: REQ-004
...
- [ ] 3.3 Implement provider priority failover logic in SmsAdapter
  - **spec_ref**: REQ-004
  - **files**: `lib/Service/SmsAdapter.php`
...
- [ ] 3.4 Implement caller-pinned provider selection (providerHint parameter)
  - **spec_ref**: REQ-004
  - **files**: `lib/Service/SmsAdapter.php`
...
- [ ] 3.5 Implement inbound webhook handling in SmsAdapter (REQ-003)
  - **spec_ref**: REQ-003
...
- [ ] 4.1 Create `lib/Service/ConsentService.php` with enforcement and recording logic
  - **spec_ref**: REQ-005
...
- [ ] 4.2 Implement automatic STOP keyword detection in webhook handlers
  - **spec_ref**: REQ-005
...
- [ ] 4.3 Implement opt-in reactivation (GIVEN opted-out contact replies "JA" to opt-in template)
  - **spec_ref**: REQ-005
  - **files**: `lib/Service/ConsentService.php`
...
- [ ] 4.4 Ensure `consent_record` is retained for GDPR erasure compliance
  - **spec_ref**: REQ-005
...
- [ ] 5.1 Create `lib/Service/BudgetService.php` with enforcement and tracking logic
...
- [ ] 5.2 Implement budget enforcement in WhatsAppAdapter and SmsAdapter send methods
  - **spec_ref**: REQ-006
  - **files**: `lib/Service/WhatsAppAdapter.php`, `lib/Service/SmsAdapter.php`
...
- [ ] 5.3 Implement alert-only budget (soft-limit) with one-per-period notification
  - **spec_ref**: REQ-006
  - **files**: `lib/Service/BudgetService.php`
...
- [ ] 5.4 Implement budget period reset logic
  - **spec_ref**: REQ-006
...
- [ ] 5.5 Implement cost capture from provider webhooks (REQ-007)
  - **spec_ref**: REQ-007
...
- [ ] 5.6 Implement cost estimation fallback for providers without cost data (Meta)
  - **spec_ref**: REQ-007
...
- [ ] 6.2 Create webhook queue handler in pipelinq (internal schema `webhook_queue`)
  - **spec_ref**: Design § Decisions § Webhook Signature Verification
...
- [ ] 6.3 Implement placeholder contact creation on first inbound (REQ-003)
  - **spec_ref**: REQ-003
  - **files**: `lib/Service/WhatsAppAdapter.php`, `lib/Service/SmsAdapter.php`
...
- [ ] 7.1 Create `lib/BackgroundJob/TemplateApprovalSyncJob.php` (hourly sync of template status)
...
- [ ] 7.2 Create `lib/BackgroundJob/BudgetPeriodResetJob.php` (daily reset of budget counters)
...
- [ ] 7.3 Create `lib/BackgroundJob/CostReconciliationJob.php` (daily reconciliation of non-EUR costs)
  - **spec_ref**: REQ-007
...
- [ ] 8.1 Add unit tests for `WhatsAppAdapter`
...
- [ ] 8.2 Add unit tests for `SmsAdapter`
...
- [ ] 8.3 Add unit tests for `ConsentService`
  - **spec_ref**: REQ-005
...
- [ ] 8.4 Add unit tests for `BudgetService`
  - **spec_ref**: REQ-006, REQ-007
...
- [ ] 8.5 Add integration tests for full send flow (WhatsApp + Consent + Budget)
...
- [ ] 8.6 Add integration tests for inbound webhook flow (all adapters)
  - **spec_ref**: REQ-003
...
- [ ] 8.7 Add integration tests for media attachment handling
  - **spec_ref**: REQ-008
...
- [ ] 9.1 Create admin documentation for provider setup (Meta, Twilio, MessageBird, CM.com)
...
- [ ] 9.2 Create agent documentation for messaging UI and session-window behavior
...
- [ ] 9.3 Update Pipelinq README to mention WhatsApp and SMS support
...
- [ ] 10.1 Run all tests and verify coverage is >= 80% for new code
  - **acceptance_criteria**:
    - All unit and integration tests pass
    - Coverage report shows >= 80% for new classes

- [ ] 10.2 Verify GDPR compliance (consent audit trail, erasure hooks)
...
- [ ] 10.3 Verify regulatory compliance (Telecommunicatiewet, DDMA, opt-out keywords)
...
- [ ] 10.4 Load-test budget enforcement and media download (5-minute Meta expiry window)
