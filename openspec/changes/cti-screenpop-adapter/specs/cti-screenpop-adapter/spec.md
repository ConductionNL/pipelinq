...
# CTI Screen-Pop and Click-to-Dial Adapter Specification

## Purpose

The CTI adapter enables Computer Telephony Integration (CTI) in Pipelinq, automating customer identification and call initiation workflows. Agents receive automatic screen-pops when calls arrive, click-to-dial for outbound calls, and automatic contact moment logging. This spec covers inbound screen-pop routing, outbound call origination, contact moment creation with disposition workflow, recording metadata attachment, and webhook integration with multiple telephony platforms.

**Demand signal**: CTI is mandatory in 98% of KCC/CRM procurement tenders.  
**Feature tier**: MVP (core screen-pop, click-to-dial, contactmoment creation, recording metadata, admin config).  
**Standards**: VNG Klantinteracties (Contactmoment), libphonenumber (E.164 normalisation), HMAC-SHA256 / OAuth 2.0 (webhook verification per platform).

## Data Model

Four new OpenRegister entities (ADR-000):

- **contactmoment** (extended): Call-specific fields added to existing schema; primary data store for all calls. Properties: `channel`, `telephony_platform`, `external_call_id`, `direction`, `from_number`, `to_number`, `started_at`, `answered_at`, `ended_at`, `duration_seconds`, `queue_name`, `agent_skill`, `disposition_subject`, `disposition_outcome`, `disposition_notes`, `recording_url`, `recording_retention_expires_at`.
- **cti_adapter_config**: Per-org telephony platform configuration. Properties: `platform`, `api_base_url`, `auth_method`, `credentials_ref`, `screen_pop_enabled`, `screen_pop_delay_ms`, `click_to_dial_enabled`, `default_outbound_caller_id`, `webhook_secret`.
- **cti_event_log**: Raw inbound webhook payloads, 30-day retention. Properties: `received_at`, `platform`, `event_type`, `external_call_id`, `payload_json`, `processed_at`, `processing_error`.
- **cti_agent_presence**: Agent availability synced from telephony platform. Properties: `user_id`, `extension`, `presence_state`, `last_updated_at`, `platform`.
