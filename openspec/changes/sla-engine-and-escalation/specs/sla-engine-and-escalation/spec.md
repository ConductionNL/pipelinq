...
# Spec: SLA Engine and Escalation

## Purpose

Define the requirements for a cross-cutting SLA engine that observes object-creation and object-update events on tracked types (request, klacht, callback, return, and future opt-in types), resolves a unique SLA policy per object, computes holiday-aware deadlines, pauses/resumes on status change, and executes escalation chains when thresholds are crossed. The engine produces immutable breach-event audit trails and aggregated attainment reporting so MKB customers can demonstrate compliance and operators can proactively escalate at-risk work.

**Main ADR refs**: [adr-000-data-model.md](../../../../architecture/adr-000-data-model.md), [adr-001-international-first-dutch-mapping.md](../../../../architecture/adr-001-international-first-dutch-mapping.md), [adr-001-data-layer.md](.claude/openspec/architecture/adr-001-data-layer.md) (register pattern, seed data, OpenRegister reuse)

**Feature tier**: P0-must (core cross-cutting capability)

**Demand evidence**: Dutch MKB service-desk compliance requirements; ITIL 4 SLA practice; ISO/IEC 20000-1:2018; NEN 7510 (zorg); AVG/GDPR Art. 33 (72u breach notification)
...
## REQ-001: Policy resolution at object creation
...
- **GIVEN** an SLA with escalation: `triggerAt: 1.5`, `notify: customer`, `channel: whatsapp`
- **WHEN** breach percentage reaches 150%
- **THEN** `OmnichanelService.preferredChannel(customer)` MUST be called to get customer's contact info
- AND the WhatsApp message MUST be dispatched with the policy's tone/template
- AND `sla_breach_event.notifiedActors` MUST include the customer's WhatsApp number (if available)

### Scenario: Webhook escalation dispatch

- **GIVEN** an SLA with escalation: `notify: webhook`, `channel: webhook`
- **WHEN** threshold is crossed
- **THEN** `WebhookService::dispatchEvent()` MUST be called with the `sla_breach_event` as CloudEvents payload
- AND the event type MUST be `nl.conduction.sla.breach`
- AND the webhook URL MUST be configurable per policy
...
- Timezones: All timestamps are stored and computed in UTC ISO 8601. Tenant display/input uses configured timezone with automatic conversion.
- Rounding: When computing business-hours, use floor() for elapsed time (e.g., 4 hours 59 minutes = 4 hours consumed, not 5).
- Leap seconds: Ignored; use standard UTC timestamps.
- Daylight saving time (DST): Handled by platform-level timezone library; no special SLA logic.

### Error Handling

- If policy resolution returns null, log and set `slaStatus: null` on the object (fail-safe; do not error).
- If holiday-calendar JSON is malformed, log and fall back to `calendar: 24x7` for that object (no escalations lost).
- If escalation channel dispatch (email, WebhookService) throws, log the error, mark `sla_breach_event.notifiedActors` with `"failed:<channel>"` placeholder, and rely on the sweep job to retry.

### Concurrency

- Event listeners fire synchronously; no queuing/async processing (REQ-007 requirement: "before API response returns").
- Sweep job is scheduled; multiple instances MUST NOT run concurrently (standard Nextcloud job locking).
- Policy CRUD can race with SLA evaluation; use optimistic locking (version field) on policy updates.
