...
- [x] 1.1 `SemanticHandoffService` (kind resolution + emit wrapper)
...
- [x] 2.1 Conversion endpoint + lifecycle wiring
  - **spec_ref**: `specs/request-management/spec.md#requirement-request-to-case-conversion-v1`
...
- [x] 2.2 Request emit declaration on the schema
  - **spec_ref**: `specs/request-management/spec.md#requirement-request-to-case-conversion-v1`
...
- [x] 2.3 "Convert to case" UI + converted state
  - **spec_ref**: `specs/request-management/spec.md#requirement-request-to-case-conversion-v1`
...
- [x] 3.1 `ns#Contract` declaration on the contract schema
...
- [x] 3.2 Contract "Send to invoicing" emit
...
- [x] 3.3 Quote declaration + emit (spec-binding only until quoting is built)
...
- [x] 4.1 Feature docs for the handoff chains
  - **spec_ref**: `specs/request-management/spec.md#requirement-request-to-case-conversion-v1`
...
## Deviations (recorded at apply time, 2026-07-07 — verified against HEAD)

The design.md dialect/API shapes were indicative; the following were adjusted to
the REAL shipped OR handoff engine (verified on ../openregister, ../hydra,
../shillinq origin/development):

- **Engine API.** `SemanticHandoffService.hasImplementer` uses OR's
  `SemanticTypeResolver::resolveSchemaByImplements($uri): ?Schema` (there is no
  `hasImplementer`/`resolveImplementer`); `handoff()` calls
  `HandoffService::execute(register, schema, id, handoffId): array` (there is no
  `handoff(kindUri, payload)` and no `HandoffResult` type). Both OR services are
  autowired; resolved lazily via the container (OR-absent safe).
- **Declarative emit.** The real engine is driven by an `x-openregister-handoff`
  schema annotation (top-level array; expressions from/const/template/semanticRef/
  provenance) — added to the `request` schema (→ ns#Case) and the `contract`
  schema (→ ns#Invoice), mirroring shillinq's shipped emitter entries. The
  `contract` schema also carries `configuration.implements: [ns#Contract]` for
  discovery.
- **Field mappings follow the REAL `HandoffKindContracts`**, not the indicative
  design table: ns#Case = title←title, summary←description, channel←channel,
  source←template URN, requester←semanticRef client (the design's name/subject/
  applicant do not exist in the contract). `source` uses a `template` scalar URN
  (shillinq's proven pattern) rather than a `provenance` object.
- **priority omitted** from the ns#Case mapping (optional field) to avoid a
  cross-enum target-validation failure (request low/normal/high/urgent vs the
  ns#Case laag/normaal/hoog); mandatory fields are fully mapped.
- **caseReference / invoiceReference are string fields** storing the created
  target UUID; app-id-level provenance additionally lives in the OR
  handoff:<id> relations the engine writes (the caseReference field is a string,
  so a `{targetUuid, implementerAppId}` object could not be stored inline).
- **Not live-verified.** No procest / shillinq is installed in this environment,
  so the actual case/invoice CREATE through the engine is not exercised
  end-to-end; `hasImplementer` correctly returns false and the actions hide
  (the tested hidden-without-implementer path). The `handoff()` wiring + the
  endpoint matrices are unit-tested with resolver/engine stubs. The shillinq
  spec also flags that merged `required` lists may fail target validation until
  the abstract-order-primitive dedup lands — an OR/shillinq-side concern.
