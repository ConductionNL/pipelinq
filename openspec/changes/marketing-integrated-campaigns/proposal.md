...
6. **A weekly review agent that only writes.** A hermiq agent template reads the campaign, social and search objects and writes one page: what moved, what to try, and three topic ideas. It is delivered as a Talk message and a card on the Reports page. It has no send tool and no publish tool, and every artefact it writes carries the ADR-088 mark.
...
- New services `SegmentSignalService`, `JourneyService`, `JourneyFlowCompiler`, `JourneyStepRunner`, `SuppressionService` and `WeeklyReviewService`.
