...
- [ ] Confirm three registry-mediated pipelinq→shillinq dispatchers already exist
...
- [ ] Confirm `PosBookkeepingService` is the only remaining surface that OWNS
...
- [ ] Confirm `posZReport` is operational commercial state (cash-drawer close /
      takings) and stays in pipelinq — only the journal-entry consequence delegates.
- [ ] Confirm `BillingApproval` is already an href to shillinq (no duplicate
      billing UI is being built here).

## Phase 1: Nav / IA correction (ADR-022)

- [ ] Remove the `ZReports` ("Boekhoudkundige Afhandeling") entry from the
      `menu` array in `src/manifest.json`.
- [ ] Remove the `PosBookkeepingSettings` ("POS bookkeeping") admin entry from
      the `menu` array in `src/manifest.json`.
- [ ] Keep the `ZReports` (`/pos/z-reports`) and `ZReportDetail`
      (`/pos/z-reports/:id`) page definitions routable in the `pages` array.
- [ ] Remove the `PosBookkeepingSettings` page (`/admin/pos-bookkeeping`) from the
      `pages` array and delete `src/views/admin/PosBookkeepingSettings.vue`.
- [ ] Relabel `BillingApproval` to `Timesheet approval` and resolve its href
...
- [ ] Replace the hard-coded `rtrim($endpoint,'/').'/api/JournalEntry'` POST in
...
- [ ] Remove the local GL-balancing build (VAT→GL line construction) from
      `PosBookkeepingService`; send the Z-report `taxBreakdown` (business facts) and
      let shillinq build the journal.
- [ ] Preserve the deterministic idempotency key `SHA256(zReport.uuid + reportDate)`
      so re-raises resolve to the same shillinq journal.
- [ ] Add a thin outcome projection to `posZReport`: `bookkeepingStatus`
      (pending/raised/failed) and `shillinqJournalEntryId`.

## Phase 3: Retire owned bookkeeping schemas

- [ ] Remove the `glAccountMapping` schema from
...
- [ ] For records already `posted`: copy `shillinqJournalEntryId` and
      `bookkeepingStatus: raised` onto the parent `posZReport`; leave the outbound
      record read-only (never drop).
- [ ] Document the one-off manual export of `glAccountMapping` profiles into
      shillinq's GL configuration.
- [ ] Ensure the repair step is re-runnable and never fails the upgrade if shillinq
      is unreachable (logs + leaves `bookkeepingStatus: pending`).

## Phase 5: i18n + verification

- [ ] Update i18n (nl + en) for the relabelled `Timesheet approval` entry and the
...
- [ ] `cd pipelinq && openspec validate pipelinq-bookkeeping-to-shillinq --strict`
      passes.
