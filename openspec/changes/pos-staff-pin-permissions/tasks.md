...
- [ ] 0.1 Search `openspec/specs/` and `openregister/lib/Service/` for existing staff auth,
  role permission, or PIN management capabilities. Document findings (even if "no overlap found")
  before writing any code.

## 1. Schema Definition

- [ ] 1.1 Add `posRole` schema to `lib/Settings/pipelinq_register.json` with properties:
...
- [ ] 1.2 Add `posStaff` schema to `lib/Settings/pipelinq_register.json` with properties:
...
- [ ] 1.3 Add both schemas to the register's `schemas` array.
- [ ] 1.4 Add seed data to `components.objects[]`: 3 posRole objects (Kassamedewerker, Supervisor,
...
- [ ] 2.1 Create `lib/Service/PosRoleService.php`:
...
- [ ] 2.2 Add `@spec openspec/changes/pos-staff-pin-permissions/tasks.md#2` PHPDoc tag to the class.

## 3. Backend — PosStaffService

- [ ] 3.1 Create `lib/Service/PosStaffService.php`:
...
- [ ] 3.2 Add `@spec` PHPDoc tag to the class.
- [ ] 3.3 Extract `authorizeStaff(object, user)` helper: per-object auth check used in all
  mutation endpoints (pattern from ADR-005).

## 4. Backend — Controllers and Routes

- [ ] 4.1 Create `lib/Controller/PosRoleController.php` with actions:
  `index`, `show`, `create`, `update`, `destroy`.
  - `index` and `show`: `#[NoAdminRequired]`
  - `create`, `update`, `destroy`: require `IGroupManager::isAdmin()` check
- [ ] 4.2 Create `lib/Controller/PosStaffController.php` with actions:
...
- [ ] 4.3 Add routes to `appinfo/routes.php`:
...
- [ ] 4.4 Add `@spec` PHPDoc tags to both controllers.

## 5. Frontend — Pinia Store

- [ ] 5.1 Register `pos-role` and `pos-staff` object types in `src/store/store.js` using
  `createObjectStore` (one each).
- [ ] 5.2 Create `src/store/modules/posSessionStore.js` using Pinia `defineStore`:
...
- [ ] 6.1 Create `src/composables/useStaffPermissions.js`:
...
- [ ] 7.1 Create `src/components/pos/PinLoginModal.vue`:
...
- [ ] 8.1 Create `src/views/pos/RoleList.vue`:
...
- [ ] 8.2 Create `src/views/pos/RoleForm.vue`:
...
- [ ] 8.3 Create `src/views/pos/StaffList.vue`:
...
- [ ] 8.4 Create `src/views/pos/StaffForm.vue`:
...
- [ ] 9.1 Add routes to `src/router/index.js`:
  - `/pos/staff` → StaffList (name: PosStaffList)
  - `/pos/roles` → RoleList (name: PosRoleList)
- [ ] 9.2 Add "POS Medewerkers" nav item to `src/navigation/MainMenu.vue` under admin settings
  section (only rendered when `isAdmin` is true).

## 10. Per-staff Sales Attribution

- [ ] 10.1 Update transaction creation logic to write `staffMemberId` from the active
  posSessionStore when creating a transaction.
- [ ] 10.2 Add a per-staff sales report endpoint: GET `/api/pos/reports/staff-sales`
  returning transactions grouped and totalled by `staffMemberId`.
- [ ] 10.3 Verify that the shillinq commission feed includes `staffMemberId` on each
  transaction line item passed to shillinq.

## 11. Verification

- [ ] 11.1 Run `npm run build` and verify zero errors.
- [ ] 11.2 Run `php vendor/bin/phpstan analyse lib/` and verify no new errors.
- [ ] 11.3 Manual browser test: create role → create staff → PIN login on POS terminal →
  attempt void (blocked) → apply discount exceeding limit (blocked) → logout.
- [ ] 11.4 Run pre-commit checklist from ADR-015 (SPDX headers, ObjectService arg counts,
