...
- [ ] 11.1 Run `npm run build` and verify zero errors. (DEFERRED: frontend
  `node_modules` is not installed in this build worktree; all new `.vue`/`.js`
  files pass standalone syntax checks (`node --check` on script blocks). Bundle
  build to be run by the reviewer/CI where node_modules is present.)
...
- [ ] 11.3 Manual browser test: create role → create staff → PIN login on POS terminal →
  ...
  (DEFERRED: requires a running Nextcloud instance with the register imported;
  the full flow is covered by unit tests on the server-authoritative logic.)
