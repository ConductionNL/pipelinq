...
- [ ] 0.1 Verify `posTransaction` and `posTransactionLine` schemas exist in `lib/Settings/pipelinq_register.json` from pos-transaction-core
- [ ] 0.2 Confirm shillinq app is available and can receive CloudEvents; test CloudEvent subscription pattern
- [ ] 0.3 Verify OpenRegister supports nested object arrays; if not, plan alternative for `posTender` relationships
- [ ] 0.4 Check whether `posTransaction.settledAt` timestamp already exists; if not, add to pos-transaction-core update

## 1. Data Model: Add posTenderType and posTender Schemas

- [ ] 1.1 Add `posTenderType` schema to `lib/Settings/pipelinq_register.json`:
...
- [ ] 1.2 Add `posTender` schema to `lib/Settings/pipelinq_register.json`:
...
- [ ] 1.3 Add seed data for 3 `posTenderType` objects (CASH, CARD, VOUCHER) with Dutch names and correct GL accounts
...
- [ ] 1.4 Add seed data for 5 `posTender` objects linked to existing posTransaction seeds
  - Include examples of: cash only, split cash+card, cash with change, voucher

## 2. Backend Service: PosPaymentService

- [ ] 2.1 Create `lib/Service/PosPaymentService.php`

- [ ] 2.2 Implement `getTenderTypeByCode(string $code): array`
  - Queries `posTenderType` register for matching `code`
  - Returns single type object or throws `TenderTypeNotFoundException`

- [ ] 2.3 Implement `validateTenderSum(string $transactionId): array`
...
- [ ] 2.4 Implement `calculateChange(float $cashTenderedAmount, float $transactionTotal): float`
...
- [ ] 2.5 Implement `addTender(string $transactionId, array $tender): array`
...
- [ ] 2.6 Implement `removeTender(string $transactionId, string $tenderId): void`
...
- [ ] 2.7 Implement `getTendersForTransaction(string $transactionId): array`
...
- [ ] 3.1 In `lib/Service/PosTransactionService.php`, locate the settlement method (e.g., `confirm()` or `settle()`)

- [ ] 3.2 Before allowing transition to `settled`, add tender validation:
  ```php
...
- [ ] 3.3 After successful settlement transition, emit CloudEvent for each tender (see section 5)

## 4. Backend Controller: TenderTypeController

- [ ] 4.1 Create `lib/Controller/PosPaymentController.php`

- [ ] 4.2 Implement endpoint `GET /api/pos/tender-types`:
...
- [ ] 4.3 Implement endpoint `GET /api/pos/tender-types/{id}`:
  - Auth: `#[NoAdminRequired]`
  - Fetch single `posTenderType` by UUID
  - Return JSON object or 404 if not found

- [ ] 4.4 Implement endpoint `POST /api/pos/tender-types`:
...
- [ ] 4.5 Implement endpoint `PUT /api/pos/tender-types/{id}`:
  - Auth: admin required
  - Validate updated `code` is still unique (allow same code on same object)
  - Update via `ObjectService`
  - Return updated object

- [ ] 4.6 Implement endpoint `DELETE /api/pos/tender-types/{id}`:
  - Auth: admin required
...
- [ ] 5.1 In `PosPaymentController`, implement `GET /api/pos/transactions/{transactionId}/tenders`:
  - Auth: `#[NoAdminRequired]`
  - Fetch all tenders for transaction via `PosPaymentService::getTendersForTransaction()`
  - Return JSON array sorted by `sortOrder`

- [ ] 5.2 Implement `POST /api/pos/transactions/{transactionId}/tenders`:
  - Auth: `#[NoAdminRequired]`
...
- [ ] 5.3 Implement `DELETE /api/pos/transactions/{transactionId}/tenders/{tenderId}`:
  - Auth: `#[NoAdminRequired]`
...
- [ ] 6.1 Create `lib/Event/TenderPostedEvent.php` class implementing CloudEvents spec:
...
- [ ] 6.2 Update settlement method in `PosTransactionService`:
...
- [ ] 6.3 Create background job `lib/BackgroundJob/TenderPostedRetryJob.php`:
...
- [ ] 7.1 Open `src/views/PosTransactionDetail.vue` (or equivalent path)

- [ ] 7.2 Add Tenders section below the transaction line items:
...
- [ ] 7.3 Add "Add Tender" button that opens a modal dialog:
  - Modal fields:
...
- [ ] 7.4 Implement change display logic:
...
- [ ] 8.3 Implement edit action per row:
...
- [ ] 8.4 Implement delete action per row:
...
- [ ] 9.1 Add following keys to `l10n/en.json`:
  - `"Tenders"`
...
- [ ] 9.2 Add corresponding keys to `l10n/nl.json` with Dutch translations:
...
- [ ] 9.3 Verify no hardcoded English strings in Vue components:
  - Use `t('pipelinq', 'string')` for all UI text per ADR-007
  - Grep for hardcoded values to confirm compliance

## 10. Backend Tests

- [ ] 10.1 Create `tests/Unit/Service/PosPaymentServiceTest.php`:
...
- [ ] 10.2 Create `tests/Unit/Controller/PosPaymentControllerTest.php`:
...
- [ ] 10.3 Create `tests/Unit/Service/PosTransactionServiceTest.php` (update existing):
...
- [ ] 11.1 Create `tests/Integration/Api/PosPaymentApiTest.php`:
...
- [ ] 11.2 Create `tests/Integration/CloudEvents/TenderPostedEventTest.php`:
