...
- [ ] 1.1 `campaign` and `touchpoint` schemas, plus `campaignId` on `blast`, `firstTouch`/`lastTouch` on `lead`, and `campaignId`/`touchpointIds[]`/`invoiceRef`/`model` on `attributionLink`
...
- [ ] 1.2 A Dutch demo campaign with its blast, lead and touchpoints (ADR-111), and the `campaign`/`touchpoint` object types the frontend store needs
  - **files**: `lib/Settings/register.d/98-marketing-campaigns.json`, `src/config/objectTypes.js`

## 2. The campaign mints the links

- [ ] 2.1 `CampaignService`: read, mint the slug once, validate source and medium against the vocabulary
  - **spec_ref**: `specs/marketing-campaigns/spec.md#requirement-a-campaign-owns-its-campaign-value-and-its-channel-vocabulary`
  - **files**: `lib/Service/CampaignService.php`, `tests/Unit/Service/CampaignServiceTest.php`
- [ ] 2.2 `CampaignLinkDecorator::utmFor()` and `decorate()` take the campaign, resolving each parameter separately
...
- [ ] 2.3 `BlastService` loads the blast's campaign and passes it to the decorator
  - **files**: `lib/Service/BlastService.php`, `tests/Unit/Service/BlastServiceTest.php`
- [ ] 2.4 Settings `campaign.utm_sources` and `campaign.utm_mediums`
  - **files**: `lib/Service/SettingsService.php`, `tests/Unit/Service/SettingsServiceTest.php`

## 3. Landing page from a campaign

- [ ] 3.1 `LandingPageProvisioningService`: guarded dispatch, article and form payload, result stored, error verbatim
...
- [ ] 3.2 `POST /api/campaigns/{id}/landing-page`
...
- [ ] 4.1 `OCA\Pipelinq\Event\LandingPageFormSubmittedEvent` matching portaliq's dispatch shape
...
- [ ] 4.2 `TouchpointService`: append, read per campaign, and the nonce lookup that makes the listener idempotent
  - **files**: `lib/Service/TouchpointService.php`, `tests/Unit/Service/TouchpointServiceTest.php`
- [ ] 4.3 `LandingPageFormSubmittedListener` and its registration
...
- [ ] 5.1 `ShillinqInvoiceReader`: paid AR invoices for a customer in a window, duck-typed, nothing written
...
- [ ] 5.2 `CampaignAttributionService`: first, last and linear over the touchpoint log, and the closing basis per lead
...
- [ ] 5.3 `CampaignReportService` and `GET /api/campaigns/{id}/report`, one aggregate response
...
- [ ] 6.1 Campaigns index and campaign detail with the Create landing page action
  - **files**: `src/manifest.d/78-marketing-campaigns.json`, `src/views/marketing/CampaignDetail.vue`, `src/registry.js`
- [ ] 6.2 The campaign report page and its Reports card, with no menu entry of its own (ADR-112)
  - **spec_ref**: `specs/marketing-campaigns/spec.md#requirement-one-campaign-report-page`
  - **files**: `src/views/marketing/CampaignReport.vue`, `src/manifest.json`, `src/manifest.d/78-marketing-campaigns.json`
- [ ] 6.3 `campaignsApi.js` and the pure `campaignReport.js` shaping helpers, with vitest
  - **files**: `src/services/campaignsApi.js`, `src/services/campaignReport.js`, `tests/vitest/campaignReport.spec.js`
- [ ] 6.4 Strings in `l10n/en.json` and `l10n/nl.json`, catalogues rebuilt
  - **files**: `l10n/en.json`, `l10n/nl.json`, `l10n/*.js`

## 7. Docs and e2e

- [ ] 7.1 A campaigns section in `docs/Features/marketing.md` and a campaigns user guide
  - **files**: `docs/Features/marketing.md`, `docs/user/marketing-campaigns.md`
- [ ] 7.2 `tests/e2e/spec-coverage/marketing-campaigns.spec.ts`
