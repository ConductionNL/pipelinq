...
- [x] 7.12 Confirm Dutch state literals from the lifecycle+notification slice are removed
      ...
      literals in `lib/`). **Finding**: one remaining literal `'nieuw'` in
      `lib/Controller/PublicFormController.php:284` where it sets the default status on
      a lead created via a public intake form — this is a form-submission initial-value,
      not a lifecycle state from the lifecycle+notification slice, and intentionally
      retained. No `'gepubliceerd'` or `'openbaar'` literals found in `lib/`.
