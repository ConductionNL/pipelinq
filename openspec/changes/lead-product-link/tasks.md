...
> **ADR-037 correction (build):** Seeds were NOT added to the monolith
> `lib/Settings/pipelinq_register.json`. Per ADR-037 they live in the register
> fragment `lib/Settings/register.d/30-lead-product-link.json`
> (`components.objects[]`). The fragment also seeds 2 prerequisite `lead`
> objects so the `leadProduct.lead` slug references resolve (no lead seeds
> previously existed). `ConfigFileLoaderService` did NOT additively union
> `components.objects[]` / register `schemas[]` — its generic deep-merge
> replaced lists. The fleet-standard additive-union rule was added (extracted
> into `RegisterFragmentMerger`) with unit tests in
> `tests/Unit/Service/RegisterFragmentMergerTest.php`.

- [x] 6.1 Add 3 `productCategory` seed objects to the register (Implementatie, Training, Support & Onderhoud) using `@self` envelope with unique slugs
