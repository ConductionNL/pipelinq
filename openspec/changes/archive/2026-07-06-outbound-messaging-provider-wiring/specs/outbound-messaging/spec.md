# Outbound Messaging — Provider Wiring Spec

**Spec refs**: archived changes `2026-06-14-whatsapp-sms-channel-adapter` (adapters, schemas, inbound) and `2026-06-21-pipelinq-messaging-via-or-leaf` (OR-leaf transport), OR `MessageDispatchProvider` + `ExternalIntegrationRouter` mock mode (`../openregister` origin/development), OpenConnector seeded messaging sources (`../openconnector` origin/development `lib/Settings/register.d/`), ADR-022, ADR-037 (register fragments via Repair), `align-claims-and-first-hour` (overlay beta state — referenced, not edited)
**Standards**: Meta WhatsApp business-messaging policy (opt-in + template-gated business-initiated), Telecommunicatiewet art. 11.7 (SMS consent), VNG Klantinteracties (`Contactmoment`), Schema.org `CommunicateAction`
**Feature tier**: MVP unless stated otherwise

## Provider decision (normative context)

Primary **SMS** provider: **Bird (formerly MessageBird)** via the seeded `messagebird-sms` OpenConnector source (`rest.messagebird.com`) — chosen for its zero-cost **test access keys** (`test_…`) that validate real send requests without dispatching, Amsterdam HQ/EU residency, Meta BSP status, and self-serve PAYG without minimums; SMS fallback: **CM.com** via `cmcom-sms`. Default **production WhatsApp** leg (owner decision 2026-07-06, "go for gov friendly"): **CM.com BSP via `whatsapp-bsp`** — Meta Premier BSP, Breda HQ, Dutch central-government references. `whatsapp-cloud-api` (Meta direct; test number + test recipients) is the WhatsApp CI/dev harness leg and an explicit per-tenant opt-in alternative. A Bird-native WhatsApp source (OR `ALLOWED_SOURCES` addition + OpenConnector source seed) is consciously deferred (2026-07-06) — out of scope, no follow-up issue filed.
