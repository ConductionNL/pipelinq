...
- [x] 0.1 Search `openspec/specs/` for any existing reporting/analytics specs that overlap with KPI aggregation, channel distribution, or agent performance — document findings in PR description
- [x] 0.2 Search `openregister/lib/Service/` for `ReportingService`, `AnalyticsService`, or `KpiService` — verify no existing platform service covers FCR / SLA / avg-duration calculations before building custom logic
- [x] 0.3 Verify `CnDashboardPage`, `CnChartWidget`, `CnStatsBlock`, and `CnMassExportDialog` are used for all dashboard, chart, KPI, and export UI respectively — no custom equivalents built

## 1. Backend Service

- [x] 1.1 Create `lib/Service/ReportingService.php` with the following public methods:
...
- [x] 1.2 Create `lib/Controller/ReportingController.php` with thin methods (< 10 lines each):
...
- [x] 2.1 Add 5 reporting API routes to `appinfo/routes.php`:
...
- [x] 4.1 Add rapportage routes to `src/router/index.js` (history mode, all named, no `/settings` route):
...
- [x] 5.1 Add all new translation keys to `l10n/en.json` (English identity-mapped)
- [x] 5.2 Add Dutch translations for all new keys to `l10n/nl.json`
...
- [x] 6.1 Create `tests/Unit/Service/ReportingServiceTest.php` with ≥ 3 test methods:
...
- [x] 6.2 Add Newman / Postman collection entries in `tests/integration/` for all 5 endpoints:
...
- [x] 6.3 Add browser test scenario for REQ-CR-001 (dashboard loads with KPI cards) per ADR-008

## 7. Verification

- [x] 7.1 Run `npm run build` — verify no errors
- [x] 7.2 Run `composer check:strict` — verify all PHPUnit tests pass
- [x] 7.3 Call each new API endpoint with `curl` — verify response shape and HTTP status
- [x] 7.4 Verify `PUT /api/rapportage/sla` returns HTTP 403 for a non-admin user
- [x] 7.5 Pre-commit checklist (per ADR-015):
