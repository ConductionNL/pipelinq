...
<!-- No behavior change — rename only: KlantbeeldSummaryService → Customer360SummaryService -->
...
`ClientDetail` page in `src/manifest.json`, bound to the summary endpoint at
`GET /api/customer-360/summary`. The customer 360 MVP SHALL reuse the declarative
detail machinery (default object data widget, `relatedCollections`,
`ActivityTimeline`, `ContactmomentQuickLog`) and SHALL NOT introduce a bespoke
`ClientDetail.vue` host component (ADR-062, declarative-view-system).

<!-- No behavior change — rename only: /api/klantbeeld/summary → /api/customer-360/summary -->
...
### Requirement: Quick actions from customer 360
...
#### Scenario: Create a request from customer 360
...
#### Scenario: Add a contact person from customer 360
