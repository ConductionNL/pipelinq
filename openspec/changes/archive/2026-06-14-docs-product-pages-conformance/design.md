...
This change is documentation-only. No PHP, no Vue, no OpenRegister schema changes. No new entities are introduced and no existing entity schemas are modified. All work targets the `docs/` directory, `docs/package.json`, and `docs/docusaurus.config.js`.
