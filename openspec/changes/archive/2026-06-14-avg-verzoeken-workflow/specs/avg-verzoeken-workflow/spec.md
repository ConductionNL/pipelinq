# Spec delta — avg-verzoeken-workflow (HANDOFF)

> **Status:** HANDOFF. AVG / GDPR Article 15/16/17/18/20 request handling is
> already covered (or planned for coverage) by sibling work in the fleet:
> **docudesk** (PDF render, PAdES-LTV signing, secure delivery, audit trail),
> **rekenkamer-audit-pack** (5-year dossier retention, signed evidence
> bundles, AP-style escalation export), and broader **privacy / FG / DPIA**
> initiatives (pattern detection, FG notification, Procest improvement
> linking). Building a parallel implementation in pipelinq would duplicate
> capabilities that already live closer to their canonical home per
> ADR-022 (consume the closest leaf; do not re-implement).
>
> This delta records the requirements as **deferred to handoff** so the
> intent is captured for the receiving capability owners. No pipelinq code
> is implemented by this change; all tasks are marked `[~]` HANDOFF in
> `tasks.md`.
