# Pipelinq POS Navigation Grouping — Delta Spec

**Status:** proposed
**Scope:** pipelinq
**Tier:** V1
**Depends on:** none (manifest-v2 `children[]` capability + existing routable pages)

## Purpose

Regroup pipelinq's flat run of point-of-sale top-level navigation entries into a "Point of Sale"
runtime group, a "Catalog" master-data group, and a Settings-foldout cluster for POS
configuration — using the existing manifest-v2 `children[]` nesting (ADR-037 fragments;
ADR-012 reuse, no new capability) — while keeping every page routable.
