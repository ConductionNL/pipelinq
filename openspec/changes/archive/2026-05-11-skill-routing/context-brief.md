# Context Brief: Skill-Based Routing Specification

**App:** Pipelinq — CRM and customer interaction
**Spec:** skill-routing
**Spec Type:** extension (extends `queue-management`)
**Platform:** Nextcloud + OpenRegister

## EXTENSION CONTEXT — you are extending `queue-management`

This spec is a extension of the **Queue Management Specification** capability. Do NOT define new entities or build new CRUD — reuse what `queue-management` already provides. Your job is to add configuration, seed data, or workflow templates on top of that capability.

**Why this is an extension:** Skill-based routing is a configuration/behavior layer on top of queue management — it routes incoming interactions to agents based on skills, which extends queue assignment logic rather than introducing new entities.

### Parent capability overview (excerpt from `queue-management` brief):

# Context Brief: Queue Management Specification
...
## Features (3 total, spec-linked, sorted by market demand)
...
- Idempotency: re-importing with `force: false` MUST NOT create duplicates. Match by slug
  using `ObjectService::searchObjects` with `_rbac:...

---

**Depends on:** queue-management
