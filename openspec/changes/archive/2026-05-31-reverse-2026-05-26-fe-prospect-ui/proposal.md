# Reverse-spec — Prospect UI

Retroactively specifies the observed behavior of 1 method(s) implementing prospect discovery cards. The code already exists and is exercised in the running app — this change documents its true capabilities as REQs and annotates each method with an `@spec` reference. No code logic changes.

## Affected code units

- `src/components/ProspectCard.vue::scoreClass`
