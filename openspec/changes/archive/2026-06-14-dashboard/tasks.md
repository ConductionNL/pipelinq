...
> **Status: SHIPPED.** All 91 tasks below are marked `[x]` — the implementing
> work was already merged to `origin/development` via the earlier `feature/dashboard`
> branch (see commits `e8e2b2ce` and the 5 preceding `dashboard:` commits — backend
> services + controllers + Vue widgets + i18n + unit tests + Newman collections).
> Vue widgets live under `src/views/analytics/` and `src/views/dashboard/widgets/`
> rather than the originally-spec'd `src/components/widgets/` path.
> Reclassified from `[~]` to `[x]` by VERIFY-AS-SHIPPED phase 2026-06-11.
