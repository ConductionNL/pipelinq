...
**Spec refs**: hydra ADR-046 (portaliq external portal) + contract v2 (2026-07-06 amendment — verify against HEAD at apply time), ADR-005 (server-derived subject, never trust the client), ADR-022 (apps consume OR abstractions)
**Standards**: eIDAS assurance vocabulary for `minTrust` (`low` | `substantial` | `high`)
...
- **AND** it MUST NOT contain a `contactmoment` collection (internal `notes` would leak — see design.md exclusions)
...
- **AND** it MUST NOT contain `booking` (staff-only `internalNotes` — see design.md) or any `berichtenboxMessage` inbox (BSN-scoped, not a contact/customer UUID ref)
- `@e2e exclude` portal rendering happens in portaliq, not pipelinq CI; manifest shape pinned by PHPUnit
