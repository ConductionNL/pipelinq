...
- [ ] 0.1 Confirm the OpenRegister `integration-xwiki` leaf is shipped (XwikiProvider + tab + widget + reference chip; storage='external' via OpenConnector) and note its key `xwiki`.
  - **acceptance_criteria**:
...
- [ ] 1.1 Remove the kennisbank views/components/store/editor and routes/controllers.
...
- [ ] 1.2 Retire `kennisartikel`, `kenniscategorie`, `kennisfeedback` schemas in `lib/Settings/pipelinq_register.json`.
...
- [ ] 1.3 Supersede the bespoke `xwiki-integration` change (do not build proxy/widget/sidebar).
...
- [ ] 2.1 Add `xwiki` to `linkedTypes` on `client`, `lead`, `request`.
...
- [ ] 3.1 Place the xwiki leaf tab in detail sidebars and the widget; declare `openconnector` dependency; import the xWiki source template.
...
- [ ] 4.1 Record the existing-content migration as a separate follow-up.
...
- [ ] 5.4 Confirm the kennisbank views/components/store/schemas and the bespoke xwiki proxy are gone.
