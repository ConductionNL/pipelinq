...
- [ ] 0.2 Widen `caseReference` to an ADR-048 semantic reference and drop the `procest` app id from the schema (V1)
...
- [ ] 2.2 Register `pipelinq-contact-moments-panel` on both halves with the filtered list, the widget and the quick-log form with `direction` (V1)
...
- [ ] 3.3 Hand the leaf ids and the `create` payload to dossiq for its Communication tab and the retirement of `customerContact` and the `kcc-werkplek` copy
