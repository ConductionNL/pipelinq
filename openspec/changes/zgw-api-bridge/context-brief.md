...
status: draft
---
# ZGW API (Zaakgericht Werken) REST bridge
...
Where the StUF-ZKN/BG adapter covers the legacy SOAP estate, the ZGW API bridge is pipelinq's first-class integration into the modern Dutch zaakgericht-werken stack: a set of OAuth2-secured REST APIs published by VNG Realisatie that every nieuw zaaksysteem (OpenZaak, RxMission, Decos JOIN ZGW-edition, Roxit Squit20/20 ZGW, Open Zaak SaaS bij Dimpact, ...) implements. The bridge speaks the five canonical ZGW component APIs — Zaken (ZRC), Documenten (DRC), Besluiten (BRC), Catalogi (ZTC) and Autorisaties (AC) — plus the Notificaties Routerings Component (NRC) for event-driven updates. It coexists with the StUF adapter so that a single pipelinq install can serve a fleet of gemeenten: each gemeente is bound to either ZGW or StUF for a given domain, and the bridge is the preferred path whenever both are available. This spec defines the JWT minting, the resource-by-resource client behaviour, the NRC abonnement lifecycle, the ZTC consumption pattern, and the explicit coexistence rules that prevent double-registration when a gemeente runs both stacks during a migration.

## Data Model

The bridge introduces four register entities: `ZgwEndpoint` (per-component connection profile, since ZRC/DRC/BRC/ZTC/AC may run on different hosts), `ZgwClient` (the JWT credentials, since one application can hold multiple client identities), `NrcAbonnement` (the notification subscription state) and `ZgwResourceMapping` (the bidirectional link between pipelinq's id and the ZGW URL — URLs are the canonical identifier in ZGW, not UUIDs). Existing pipelinq entities (Request, Contact, Document) reference these by id.
...
JWT credentials per VNG-API-Common spec; one client may be authorised on multiple endpoints.
...
### Requirement: REQ-ZGW-001 OAuth2 / JWT client authentication
...
### Requirement: REQ-ZGW-002 Zaken API (ZRC) resource client
...
### Requirement: REQ-ZGW-003 Documenten API (DRC) resource client
...
### Requirement: REQ-ZGW-004 Besluiten API (BRC) resource client
...
### Requirement: REQ-ZGW-005 Catalogi API (ZTC) consumption and caching
...
### Requirement: REQ-ZGW-006 Autorisaties API (AC) consumption
...
### Requirement: REQ-ZGW-007 Notificaties (NRC) subscription lifecycle
...
### Requirement: REQ-ZGW-008 Coexistence with stuf-zkn-bg-adapter
...
### Requirement: REQ-ZGW-009 ETags, optimistic concurrency and PATCH
...
### Requirement: REQ-ZGW-010 Rol mapping pipelinq Contact ↔ ZGW rol
...
- **ZGW API standaarden** — VNG Realisatie referentie-implementatie. Componenten: Zaken (ZRC), Documenten (DRC), Besluiten (BRC), Catalogi (ZTC), Autorisaties (AC), Notificaties (NRC). Publicatie + OAS-specs: https://vng-realisatie.github.io/gemma-zaken/
- **VNG-API-Common** — gedeelde profielen voor JWT-authenticatie, ETag/HTTP-caching, foutformaten (RFC 7807 problem+json). https://vng-realisatie.github.io/vng-api-common/
- **Open Zaak** — open-source referentie-implementatie van ZRC/DRC/BRC/ZTC/AC (Maykin Media + Dimpact). https://open-zaak.readthedocs.io/
- **Open Notificaties** — referentie-implementatie van NRC. https://open-notificaties.readthedocs.io/
- **GEMMA Zaakgericht Werken** — architectuurkatern dat het samenspel van ZRC/DRC/BRC/ZTC/NRC voorschrijft. https://www.gemmaonline.nl/
- **Common Ground** — vijflaagse architectuur; ZGW APIs zijn laag-3 (services on top of laag-1 register data); pipelinq als interactielaag zit in laag-5.
- **NL API Strategie / NLGov** — algemene API-ontwerpregels van de Nederlandse overheid (paging, filtering, error formats). https://docs.geostandaarden.nl/api/
- **NEN-ISO/IEC 27001 + BIO** — informatiebeveiliging; JWT-secret in vault, mutual TLS optie per endpoint.
- **AVG / UAVG** — verwerkersovereenkomst tussen pipelinq-leverancier (Conduction) en gemeente; BSN-doorgifte via rollen vereist grondslag-vermelding.

## Cross-app integration

- **request-management** (pipelinq) — Request is de bron voor createZaak; statuswijzigingen vertalen naar addStatus + updateZaak. Spec: pipelinq/openspec/specs/request-management/spec.md.
- **contactmomenten** (pipelinq) — Contact entiteit voert de bidirectionele mapping naar ZGW rollen; contactmomenten kunnen optioneel als zaakeigenschappen of als losse contactmomenten-API gepubliceerd worden (laatste is een separate VNG API buiten scope hier).
- **stuf-zkn-bg-adapter** (pipelinq, zustermodule) — Per gemeente kiest precies een van beide het actieve schrijfpad; deze bridge is verplicht-voorkeur waar een ZGW-stack beschikbaar is. Coexistentie geregeld in REQ-ZGW-008.
- **openconnector** (Conduction core) — De bridge kan worden uitgevoerd als gateway-mapping in OC; OC source/job/event-machinerie voert de afgesproken polling + queue. Spec: openconnector/openspec/specs/source-mapping/spec.md.
- **openregister** (Conduction core) — ZgwEndpoint, ZgwClient, NrcAbonnement en ZgwResourceMapping zijn schemas in het pipelinq register; OR audit log dekt mutaties op deze entiteiten, de inhoudelijke API-traffic logt de bridge zelf met dezelfde StufMessage-achtige (hier ZgwApiCall) tabel — separaat spec, hier alleen aangestipt.
- **docudesk** (DMS) — Grote bijlagen kunnen alternatief via een docudesk DMS-direct URL als EIO bestandsdelen-bron worden ingezet; standaard echter directe DRC multipart upload.
- **openzaak** (Conduction ExApp wrapper) — Voor gemeenten die de Conduction-gehoste Open Zaak willen, is de bridge het primaire pad; ExApp-side bevat geen pipelinq-logica.

## Target users

- **Zaakbehandelaar (gemeente Zoetermeer, Open Zaak / Dimpact)** — werkt in een ZGW-UI (bijv. de Dimpact medewerker-portal) en ziet de door pipelinq geregistreerde zaak met initiator-rol, documenten en eerste status binnen sekonden verschijnen.
- **KCC-medewerker (pipelinq frontend)** — registreert hetzelfde type aanvraag als bij de StUF-adapter; de pipelinq-UI is gemeente-onafhankelijk, alleen de configuratie bepaalt welk backend pad wordt gekozen.
- **Functioneel beheerder zaaksysteem** — beheert ZgwEndpoint + ZgwClient + scopes; ontvangt needs-input bij scope-tekorten of NRC-onbereikbaarheid.
- **DevOps / SRE pipelinq-leverancier (Conduction)** — verzorgt JWT-secret rotatie, monitort circuit-breakers per endpoint, valideert dat tijdens een StUF→ZGW migratie geen dubbele zaak ontstaat (REQ-ZGW-008).
- **CISO / Privacy Officer gemeente** — eist scope-minimalisme en logging van BSN-doorgifte in rollen; gebruikt de scope-cache + bridge-audit log als bewijs in DPIA.
- **VNG Realisatie reviewer** — bij Compliance Test Platform-runs (CTP) verwacht dat de pipelinq-bridge een groene run haalt op alle ZRC/DRC/BRC/ZTC/AC suites met de geconfigureerde scopes, dat NRC abonnement-bevestiging geautomatiseerd is, en dat optimistic-locking semantiek (ETags) correct wordt gehonoreerd.
- **OpenZaak / Dimpact engineer** — verwacht dat pipelinq zich gedraagt als een nette consumer: VNG-API-Common JWT, RFC 7807 fault parsing, multipart upload protocol exact gevolgd, geen polling waar NRC-notificaties beschikbaar zijn.
