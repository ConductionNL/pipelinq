...
- [ ] 0.1 Confirm `RegisterResolverService` is present in the OR-side spec and not already
      partially adopted in any other pipelinq service (grep `RegisterResolverService` in `lib/`).
- [ ] 0.2 Confirm `x-openregister-lifecycle` runtime ships in the target OR version before
      proceeding with Phase 2. Record the minimum OR version in `openspec/manifest.yaml`.
- [ ] 0.3 Confirm `ContactMatchingService` is available in OR's `pluggable-integration-registry`
...
- [ ] 1.9 Verify: run `grep -r "getValueString(APP_ID, 'register', '')" lib/` — MUST return
      zero matches.
...
- [ ] 2.2 `lib/Service/CalendarSyncService.php:76` — Add lifecycle annotation to
...
- [ ] 2.3 `lib/Controller/CallbackController.php:302` — Add lifecycle annotation to
      `request` (callback) schema: states `open → claimed → completed | cancelled`.
      Replace `'status' => 'open'` literal with lifecycle initial-state.
- [ ] 2.4 `lib/Service/AutomationService.php:220,249` — Add lifecycle annotation to
...
- [ ] 2.5 Document state transition rules and per-transition authorization in each lifecycle
...
- [ ] 3.1 `lib/Service/NotificationService.php:405-412` — remove direct
...
- [ ] 3.2 `lib/Service/ActivityService.php:291` — remove `setSubject()` call for stage-change
      events. Replace with notification annotation on the `lead` schema's stage-change
      lifecycle transition hook.
- [ ] 3.3 Verify: no `notificationManager->notify()` calls remain in `lib/Service/` for
...
- [ ] 4.1 Inventory pipelinq schemas that need Archiefwet retention:
      candidates are `automationLog` (audit log), `kennisartikel` (versions), `task` history,
      `request` (callback logs). Confirm scope with DPO.
- [ ] 4.2 Add `x-openregister-archival.retention` per schema where required. Document the
      retention period and legal basis for each.
- [ ] 4.3 Verify archival annotations validate against ADR-024 schema definition.
...
- [ ] 5.2 `openspec/specs/lead-management/spec.md:505` — declare `staleness` as a calculation
      annotation on the `lead` schema (days since last stage transition).
- [ ] 5.3 `openspec/specs/lead-management/spec.md:519` — declare `aging` as a calculation
      annotation on the `lead` schema (days since `createdAt`).
- [ ] 5.4 `openspec/specs/lead-management/spec.md:924` — declare `leadValue` as a calculation
...
- [ ] 6.4 Reference `openspec/architecture/adr-000-data-model.md` (already reframed by
...
- [ ] 7.2 `lib/BackgroundJob/QueueOverflowJob.php:41` — `INTERVAL = 300` →
      admin-config `pipelinq.queue_overflow.poll_interval_seconds` (default `300`).
- [ ] 7.3 `lib/BackgroundJob/TaskExpiryJob.php:43` — `INTERVAL = 900` →
...
- [ ] 7.7 `lib/Service/TaskService.php:73` — `BUSINESS_HOUR_START = 8` →
...
- [ ] 7.8 `lib/Service/TaskService.php:80` — `BUSINESS_HOUR_END = 17` →
...
- [ ] 7.12 Add `validateAdminConfig` bounds-checking: reject
      `poll_interval_seconds < 60`, `business_hour_start < 0`, `business_hour_start >= business_hour_end`,
      `review_interval_days < 1`.
- [ ] 7.13 Verify Dutch state literals from Phase 2 are removed from source after lifecycle
...
- [ ] 8.1 Create `openspec/manifest.yaml` with:
      `tier: 3`,
      ...
      contacts-actions]`.
- [ ] 8.2 Pin minimum OR version in the manifest — MUST include `register-resolver-service`
      and `contacts-actions` integration provider.
- [ ] 8.3 Declare `pipelinq.role: object-store-exemplar` (or equivalent key per
      `adopt-app-manifest` schema) so other apps can locate the reference implementation.
- [ ] 8.4 Validate manifest against the Hydra manifest schema once `adopt-app-manifest` ships.
...
- [ ] 9.1 `src/store/modules/object.js` — declare `multi-tenancy-context` dependency
...
- [ ] 9.2 Adopt `i18n-source-of-truth` for translatable fields on `kennisartikel` (`title`,
...
- [ ] 9.3 Adopt `i18n-api-language-negotiation` for the pipelinq API: read the
...
- [ ] 10.1 Confirm REQ-POR-012 in `openspec/changes/pipelinq-adopt-or-abstractions/specs/pipelinq-or-adoption/spec.md`
...
- [ ] 10.2 Add a cross-reference in `openspec/specs/openregister-integration/spec.md`
      pointing to REQ-POR-012 as the exemplar citation. Do NOT rewrite that spec.
- [ ] 10.3 Verify: no commit in the apply phase touches `src/store/modules/object.js`.
...
- [ ] S.1 Update seed `kennisartikel` objects to use valid lifecycle state names
      (`nieuw`, `gepubliceerd`, `ingetrokken`) and to have `visibility` as a separate field
      (`openbaar`, `intern`), not mixed with status.
- [ ] S.2 Update seed `automationLog` objects to use valid lifecycle state names
      (`pending`, `running`, `succeeded`, `failed`, `skipped`).
- [ ] S.3 Verify seed data import is idempotent: re-run `importFromApp()` and confirm
