...
**Implemented:**
- LeadProduct entity is defined in `lib/Settings/pipelinq_register.json` as a `leadProduct` schema within the `pipelinq` register, registered alongside client, contact, lead, request, pipeline, product, and productCategory schemas.
- Frontend store registration for `leadProduct` is implemented in `src/store/store.js` (line 32-34: `objectStore.registerObjectType('leadProduct', config.leadProduct_schema, config.register)`).
...
- CRUD operations use the shared `useObjectStore` from `src/store/modules/object.js` which delegates to the `@conduction/nextcloud-vue` library's `createObjectStore`.
- Product pre-population: `onProductSelect()` copies `unitPrice` from the selected product to the line item form.

**Not yet implemented:**
- Discount calculation uses subtraction (`qty * price - discount`) instead of the spec's percentage formula (`qty * price * (1 - discount/100)`). This is a bug relative to the spec.
- Lead value auto-recalculation on the pipeline board and lead list views is not implemented -- the LeadProducts component emits `value-changed` and `sync-value` events, but the auto-calculation (sum of line items overriding lead value) is only advisory, not automatic.
- SKU search in the Add Product dialog is not implemented -- only product name matching via NcSelect label.
- The `notes` field on LeadProduct line items is saved but not displayed in the table or editable inline.

**Partial implementations:**
- The manual override hint is implemented but uses slightly different wording than the spec ("Lead value is manually set to {manual}. Calculated total: {calculated}." vs spec's "Manual override (products total: EUR 5,000)").
...
- The spec is specific enough to implement as-is. Scenarios cover the complete lifecycle of line items including add, edit, remove, and value recalculation.
- **Ambiguity:** The spec says discount is a "percentage (0-100)" but the current implementation treats it as an absolute value. The spec is correct; the implementation needs fixing.
- **Open question:** Should the lead value auto-update without user confirmation when line items change, or should it require explicit "sync" action? The spec says "MUST automatically recalculate" (REQ Lead Value Auto-Calculation, Scenario: Lead value update on line item change), which suggests automatic behavior.
- **Missing:** No mention of currency configuration (assumes EUR). Should this be configurable per organization?
