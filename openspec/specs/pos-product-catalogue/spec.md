...
**Status**: active
**Capability**: pos-product-catalogue
**Schema.org mapping**: `schema:Product` (product), `schema:gtin` (barcode)

**OpenSpec changes**:
- `pos-product-catalogue` (archived 2026-05-31) — Initial: extend the `product` schema into a POS-grade product master (variants matrix, modifier groups, quantity price tiers, explicit Dutch BTW class, barcode/EAN, service duration); server-authoritative ProductCatalogService (BTW class→rate, effective price for qty+variant, scoped barcode lookup) + ProductCatalogController endpoints; Vue variant/modifier/price-tier editors, BTW-class selector, barcode scan view; 5 Dutch seed products.
...
- AND the server MUST treat the BTW class as the source of truth for the rate (`ProductCatalogService::btwClassToRate`), failing closed to the 21% high rate for an unknown or empty class.
...
- THEN the lookup MUST resolve server-side (`POST /api/products/barcode-lookup`), scoped to this app's own register + product schema
...
- THEN the lookup MUST return 404 and the view MUST show an empty state: "Geen product gevonden voor barcode 9999999999999"
