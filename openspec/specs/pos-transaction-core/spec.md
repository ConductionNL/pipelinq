...
**Status**: active
**Capability**: pos-transaction-core
**Schema.org mapping**: `schema:Order` (posTransaction), `schema:OrderItem` (posTransactionLine)

**OpenSpec changes**:
- `pos-transaction-core` (archived 2026-05-31) — Initial: two OR schemas (posTransaction lifecycle draft→parked→confirmed→settled→refunded with per-rate tax breakdown + grand total; posTransactionLine qty/discount/computed tax), PosTransactionService (server-authoritative totals, lifecycle, CloudEvent to shillinq), PosTransactionController, Vue list/detail/cart views.
