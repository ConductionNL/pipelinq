...
**Status**: active
**Capability**: pos-refund-return
**Schema.org mapping**: `schema:Order` (type=refund), `schema:OrderItem` (type=refund-line), `schema:DefinedTerm` (refundReason)

**OpenSpec changes**:
- `pos-refund-return` (archived 2026-05-31) — Initial: structured `posRefund` / `posRefundLine` / `refundReason` schemas + seed data; `PosRefundService` computing refund amounts server-side from each original `posTransactionLine`'s persisted tax (proportional, ratio-clamped), enforcing a cumulative over-refund cap, manager-gated (fail-closed) confirm/reject transitions, and emitting fire-and-forget `pipelinq.TransactionRefund.completed` (reversal, negative amounts) + per-restocked-line `shillinq.StockMovement` CloudEvents; IDOR-safe `PosRefundController` (`/api/pos-refunds/{id}/confirm|reject`); refund list/detail/form views with line-selection + per-line reason/restock + real-time totals, isolated reject dialog, nav entry "Retouren" + "Retour registreren" button on the transaction detail; nl+en i18n; 16 unit tests.

---

## Implementation notes (as built)

- The refund references its parent `posTransaction` and reuses that transaction's persisted, server-authoritative line tax data (written by `PosTransactionService` on confirm) rather than forking a parallel transaction model. The merged `PosTransactionService::refundTransaction()` (whole-transaction status flip) is left intact; this capability adds the structured partial-refund record model on top.
- All refund amounts are recomputed server-side on confirm (`PosRefundService::recalculateTotals` → `recalculateLine`); client-supplied `taxAmount`/`lineTotal` are overwritten. The proportion is clamped to `[0,1]` so an over-quantity return cannot inflate a refund.
- The over-refund cap is cumulative: confirm sums all already-`completed` refunds for the same transaction and rejects when the running total would exceed the original transaction total (one-cent rounding tolerance).
- Confirm and reject are manager-gated and fail closed (NC admin or configured `pos_manager_group`; no group configured → admins only).
- Stock back-in and accounting reversal are emitted fire-and-forget through OpenRegister's `WebhookService`. When no Shillinq / inventory consumer is subscribed the dispatch is a silent no-op — refund completion never fails on a missing downstream subscriber. The frontend "real-time totals" mirror the server formula and are never persisted as authoritative.
- Frontend routing is manifest-driven (`src/manifest.json` + `src/registry.js`), not vue-router; refund routes are placed before the `/pos/:id` wildcard so `/pos/refunds*` is not shadowed.
