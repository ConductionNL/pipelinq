...
## Requirements
### Requirement: Reporting UI — documented operations

The contactmoment reporting screens implemented in this app MUST provide the operations enumerated in this change's tasks.md (for example `fetchData`, `fetchData`, `exportCsv`, `fetchData`, `mounted`, `slaStatusClass`). Each listed method realises an observable part of contactmoment reporting screens and MUST behave as implemented in the current codebase.
...
- WHEN a caller invokes one of the documented operations for contactmoment reporting screens
- THEN the operation MUST execute and return a result consistent with the current implementation

---

### Requirement: Reporting UI — results derived from current CRM state

Operations for contactmoment reporting screens MUST read their inputs from the relevant CRM entities/configuration and compute results from that live state (no hard-coded or stubbed responses). Derivations such as formatting, aggregation, filtering and validation MUST reflect the data present at call time.

**Feature tier**: V1

#### Scenario: Results reflect live state

- GIVEN CRM data backing contactmoment reporting screens
...
### Requirement: Reporting UI — defensive handling of absent or invalid input

Operations for contactmoment reporting screens MUST tolerate missing, empty, or malformed input without throwing unhandled errors — returning empty or default results, or surfacing a validation outcome as implemented, rather than crashing the surrounding flow.

**Feature tier**: V1

#### Scenario: Missing input does not crash the flow

- GIVEN an operation for contactmoment reporting screens is called with absent or invalid input
- WHEN it executes
