...
## Purpose
Defines the security posture of the public customer portal API — a set of
`@PublicPage` endpoints authenticated per-request by a portal session token
(not a Nextcloud user), served for one or more tenants resolved from
server-trusted signals. This spec captures the cross-origin embedding boundary
enforced at the single tenant-resolution gate every portal endpoint passes
through.
