...
The `getUserLocale()` function in `src/services/localeUtils.js` (REQ-I18N-001) MUST return the active Nextcloud user locale as a BCP 47 language tag (e.g. `nl-NL`, `en-GB`, `de-DE`).
...
### Requirement: Currency formatting [MVP]

The `formatCurrency(value, currency)` function (REQ-I18N-002) MUST format numeric monetary values using the active user locale and the `Intl.NumberFormat` API with `style: 'currency'`.
...
### Requirement: Date formatting [MVP]

The `formatDate(dateStr, options)` function (REQ-I18N-003) MUST format ISO 8601 date strings using the active user locale and `Intl.DateTimeFormat`.
...
### Requirement: Relative time formatting [MVP]

The `formatRelativeTime(dateStr)` function (REQ-I18N-004) MUST return a locale-aware relative time string for ISO 8601 date strings.
...
### Requirement: Component migration — no hardcoded nl-NL [MVP]

All Vue components that previously called `toLocaleString('nl-NL', ...)` (REQ-I18N-005) MUST be updated to import and use the shared helpers from `src/services/localeUtils.js`. No component MAY contain a hardcoded `'nl-NL'` locale string after this change is implemented.
...
### Requirement: Dutch user experience unchanged [MVP]

For users with `nl-NL` locale (REQ-I18N-006), the visual output of all formatted values MUST be identical to the output produced by the previous hardcoded `toLocaleString('nl-NL', ...)` calls. No regression in Dutch formatting is acceptable.
