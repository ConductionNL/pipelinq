<template>
	<div class="qe">
		<div v-for="(q, i) in questions" :key="q.id" class="qcard">
			<div class="qhdr">
				...
				<label
					><input
						...
						@change="setProp(i, 'required', $event.target.checked)" />
					Req</label
				>
				<button
					type="button"
					:aria-label="t('pipelinq', 'Remove question {n}', { n: i + 1 })"
					@click="remove(i)">
					X
				</button>
			</div>
			<input
				:value="q.text"
				:placeholder="t('pipelinq', 'Question text')"
				:aria-label="t('pipelinq', 'Text of question {n}', { n: i + 1 })"
				@input="setProp(i, 'text', $event.target.value)" />
			<div v-if="q.type === 'multiple_choice'" class="opts">
				<div v-for="(o, oi) in q.options || []" :key="oi" class="opt-row">
					<input
						:value="o"
						:placeholder="t('pipelinq', 'Option {n}', { n: oi + 1 })"
						:aria-label="t('pipelinq', 'Option {n}', { n: oi + 1 })"
						@input="setOpt(i, oi, $event.target.value)" />
					<button
						v-if="(q.options || []).length > 2"
						type="button"
						:aria-label="
							t('pipelinq', 'Remove option {n}', { n: oi + 1 })
						"
						@click="rmOpt(i, oi)">
						x
					</button>
...
export default {
	...
	methods: {
		...
		setType(i, t) {
			...
			if (
				t === 'multiple_choice'
				&& (!a[i].options || a[i].options.length < 2)
			)
				a[i].options = ['', '']
			this.emit(a)
		},
