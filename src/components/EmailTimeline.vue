		...
		<div v-else class="email-timeline__list">
			<div
				...
				<div class="email-item__icon">
					<span v-if="email.direction === 'inbound'" class="direction-icon"
						>&#x2199;</span
					>
				...
				<div class="email-item__content">
					<div class="email-item__header">
						<span class="email-item__subject">{{
							email.subject || t('pipelinq', '(no subject)')
						}}</span>
						<span class="email-item__date">{{
...
.email-timeline__empty {
	padding: 16px;
	text-align: center;
	color: var(--color-text-lighter);
}

.email-timeline__list {
	display: flex;
