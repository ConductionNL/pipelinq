...
<template>
	<div class="portal-password-reset">
		...
		<form v-else @submit.prevent="doReset">
			<div class="portal-field">
				<label for="portal-new-password">{{ t('pipelinq', 'New password') }}</label>
				<input id="portal-new-password"
					v-model="password"
					type="password"
...
export default {
	...
	data() {
		...
		return { email: '', password: '', token: params.get('token') || '', message: '', error: '' }
	},
	methods: {
		async requestReset() {
			this.message = ''
			...
			try {
				...
				this.message = t('pipelinq', 'If that email address exists, a reset link has been sent.')
			} catch (e) {
		...
		async doReset() {
			...
			try {
				...
				this.message = t('pipelinq', 'Your password has been reset. You can now sign in.')
				setTimeout(() => this.$router.push('/login'), 2000)
			} catch (e) {
				this.error = e.message || t('pipelinq', 'Could not reset your password.')
			}
		},
	},
}
