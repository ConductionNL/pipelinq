		...
		<template v-else>
			<section v-if="series.length >= 1" class="forecast-trend__chart">
				...
				<div class="forecast-trend__legend">
					<span class="legend legend--commit">{{ t('pipelinq', 'Commit') }}</span>
					<span class="legend legend--bestcase">{{ t('pipelinq', 'Best Case') }}</span>
					<span class="legend legend--pipeline">{{ t('pipelinq', 'Pipeline') }}</span>
				</div>
			</section>

			<NcEmptyContent v-else
				:name="t('pipelinq', 'Not enough snapshots')"
				:description="t('pipelinq', 'At least one weekly snapshot is needed to show a trend.')" />

			<section v-if="delta" class="forecast-trend__delta">
				<h3>{{ t('pipelinq', 'Deal movements (week over week)') }}</h3>
				<p>{{ t('pipelinq', 'Moved into commit') }}: {{ delta.intoCommit || t('pipelinq', 'none') }}</p>
				<p>{{ t('pipelinq', 'Moved out of commit') }}: {{ delta.outOfCommit || t('pipelinq', 'none') }}</p>
			</section>
...
export default {
	...
	methods: {
		async loadTrend() {
			...
			this.loading = true
			try {
				const data = await fetchSnapshots({ periodId: this.periodId, level: 'rep', ownerId: this.ownerId, limit: 100 })
		...
		buildDelta() {
			...
			this.delta = {
				intoCommit: curr.commit > prev.commit ? this.formatMoney(curr.commit - prev.commit) : '',
				outOfCommit: curr.commit < prev.commit ? this.formatMoney(prev.commit - curr.commit) : '',
			}
		},
		line(key) {
