...
<template>
	<div class="pos-role-form">
		<div class="pos-role-form__header">
			...
			<h2>{{ isNew ? t('pipelinq', 'New POS role') : (form.name || t('pipelinq', 'POS role')) }}</h2>
		</div>
...
export default {
	...
	computed: {
		...
		canSave() {
			return !!this.form.name && this.form.maxDiscountPercent >= 0 && this.form.maxDiscountPercent <= 100
		},
	},
	async mounted() {
		if (!this.isNew) {
	...
	methods: {
		async load() {
			...
			try {
				...
				this.errorMessage = error?.response?.data?.error || t('pipelinq', 'Failed to load role')
			} finally {
				this.loading = false
			}
		},
