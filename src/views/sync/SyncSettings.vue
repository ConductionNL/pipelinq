		...
		<div class="sync-settings__section">
			...
			<p class="sync-settings__description">
				{{
					t(
						'pipelinq',
						'When enabled, emails from selected accounts are automatically linked to CRM contacts by matching email addresses.',
					)
				}}
			</p>
				...
				<div class="sync-settings__accounts">
					...
					<p v-if="mailAccounts.length === 0" class="empty-hint">
						{{
							t(
								'pipelinq',
								'No mail accounts found. Configure email accounts in Nextcloud Mail first.',
							)
						}}
					</p>
					<div
				...
				<div class="sync-settings__status">
					...
					<div class="status-info">
						<span
							...
							{{ lastSyncTime || t('pipelinq', 'Never') }}</span
						>
						<span
				...
				<div class="sync-settings__privacy">
					...
					<p class="privacy-notice">
						{{
							t(
								'pipelinq',
								'Email metadata (subject, sender, date) is stored in the CRM. Full email content is accessed on-demand from Nextcloud Mail and is only visible to the syncing user by default.',
							)
						}}
					</p>
				</div>
		...
		<div class="sync-settings__section">
			...
			<p class="sync-settings__description">
				{{
					t(
						'pipelinq',
						'Follow-ups and meetings created in Pipelinq appear in your Nextcloud Calendar. Calendar events with known contacts are linked to their CRM profile.',
					)
				}}
			</p>
			...
			<div v-if="calendarSyncEnabled" class="sync-settings__calendar-info">
				<p>
					{{
						t(
							'pipelinq',
							'Events will be created in a dedicated "Pipelinq" calendar in your Nextcloud Calendar.',
						)
					}}
				</p>
			</div>
