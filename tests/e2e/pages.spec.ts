...
test.describe.skip('Clients page', () => {
	...
	test('Add Item navigates to new client form', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: 'New client', level: 2 }),
		).toBeVisible()
	})
...
test.describe.skip('Leads page', () => {
	...
	test('new lead form has correct fields', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: 'New lead', level: 2 }),
		...
		await expect(
			page.getByRole('spinbutton', { name: 'Value (EUR)' }),
		).toBeVisible()
		await expect(
			page.getByRole('spinbutton', { name: 'Probability %' }),
		).toBeVisible()
		await expect(
			page.getByRole('combobox', { name: 'Select source' }),
		).toBeVisible()
		await expect(
			page.getByRole('textbox', { name: 'Expected close date' }),
		).toBeVisible()
		await expect(
			page.getByRole('combobox', { name: 'Select client' }),
		).toBeVisible()
		await expect(
			page.getByRole('combobox', { name: 'Select pipeline' }),
		).toBeVisible()
		// Stage disabled until pipeline selected
		await expect(
			page.getByRole('combobox', { name: 'Select pipeline first' }),
		).toBeDisabled()
		await expect(page.getByRole('button', { name: 'Create' })).toBeDisabled()
	})
})

test.describe.skip('Requests page', () => {
	// TODO(#392): rewrite for manifest-driven app shell
	...
	test('new request form has correct fields', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: 'New request', level: 2 }),
		).toBeVisible({ timeout: 10000 })
		await expect(page.getByRole('textbox', { name: 'Title' })).toBeVisible()
		await expect(
			page.getByRole('textbox', { name: 'Description' }),
		...
		await expect(
			page.getByRole('textbox', { name: 'Requested at' }),
		).toBeVisible()
		await expect(
			page.getByRole('combobox', { name: 'Select client' }),
		).toBeVisible()
...
test.describe.skip('Pipeline page', () => {
	...
	test('renders pipeline view with correct controls', async ({ page }) => {
		...
		await expect(
			page.getByRole('button', { name: 'Pipeline settings' }),
		).toBeVisible()
	})
	...
	test('shows empty state with New pipeline button', async ({ page }) => {
		await page.goto('/apps/pipelinq/pipeline')
		await expect(page.getByText('No pipeline selected')).toBeVisible({
			timeout: 10000,
		})
		await expect(
			page.getByRole('button', { name: 'New pipeline' }),
		).toBeVisible()
	})
})

test.describe.skip('My Work page', () => {
	// TODO(#392): rewrite for manifest-driven app shell

	test('renders with correct filter controls', async ({ page }) => {
		await page.goto('/apps/pipelinq/my-work')
		await expect(
			page.getByRole('heading', { name: 'My Work', level: 2 }),
		).toBeVisible({ timeout: 10000 })
...
test.describe.skip('Tasks page', () => {
	...
	test('new task form has correct fields', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: 'New task', level: 2 }),
		).toBeVisible({ timeout: 10000 })
...
test.describe.skip('Contactmomenten page', () => {
	// TODO(#392): rewrite for manifest-driven app shell

	test('renders with heading and action buttons', async ({ page }) => {
		await page.goto('/apps/pipelinq/contactmomenten')
		await expect(
			page.getByRole('heading', { name: 'Contact Moments', level: 2 }),
		).toBeVisible({ timeout: 10000 })
		await expect(
			page.getByRole('button', { name: 'New contact moment' }),
		).toBeVisible()
	...
	test('new contactmoment form has channel buttons', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: /Register Contact Moment/i }),
		).toBeVisible({ timeout: 10000 })
		await expect(page.getByRole('textbox', { name: 'Subject' })).toBeVisible()
...
test.describe.skip('Complaints page', () => {
	// TODO(#392): rewrite for manifest-driven app shell
	...
	test('new complaint form has correct fields', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: 'New complaint', level: 2 }),
		).toBeVisible({ timeout: 10000 })
		await expect(page.getByRole('textbox', { name: 'Title' })).toBeVisible()
		await expect(
			page.getByRole('textbox', { name: 'Description' }),
		...
		await expect(
			page.getByRole('combobox', { name: 'Search client' }),
		).toBeVisible()
		await expect(page.getByRole('button', { name: 'Create' })).toBeDisabled()
	})
})

test.describe.skip('Surveys page', () => {
	// TODO(#392): rewrite for manifest-driven app shell

	test('renders with heading and create button', async ({ page }) => {
		await page.goto('/apps/pipelinq/surveys')
		await expect(
			page.getByRole('heading', { name: 'Surveys', level: 2 }),
		).toBeVisible({ timeout: 10000 })
...
test.describe.skip('Queues page', () => {
	// TODO(#392): rewrite for manifest-driven app shell

	test('renders with heading and create button', async ({ page }) => {
		await page.goto('/apps/pipelinq/queues')
		await expect(
			page.getByRole('heading', { name: 'Queues', level: 2 }),
		).toBeVisible({ timeout: 10000 })
	...
	test('Add queue opens modal with correct fields', async ({ page }) => {
		...
		await expect(
			page.getByRole('heading', { name: 'Create queue' }),
...
test.describe.skip('Reporting page', () => {
	...
	test('has analytics tabs', async ({ page }) => {
		await page.goto('/apps/pipelinq/rapportage')
		await expect(
			page.getByRole('button', { name: 'Channel Analytics' }),
		).toBeVisible({ timeout: 10000 })
		await expect(
			page.getByRole('button', { name: 'Agent Performance' }),
		).toBeVisible()
	})
})

test.describe.skip('Pipelines settings page', () => {
	// TODO(#392): rewrite for manifest-driven app shell
	...
	test('renders with heading and create button', async ({ page }) => {
		...
		await expect(
			page.getByRole('button', { name: 'Add pipeline' }),
		).toBeVisible()
	})

	test('shows empty state prompting pipeline creation', async ({ page }) => {
		await page.goto('/apps/pipelinq/pipelines')
		await expect(page.getByText('No pipelines configured')).toBeVisible({
			timeout: 10000,
		})
		await expect(
			page.getByRole('button', { name: 'Create first pipeline' }),
		).toBeVisible()
	})
})
