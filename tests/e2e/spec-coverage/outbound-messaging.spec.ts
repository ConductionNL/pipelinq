...
async function assertNoServerError(page) {
	...
	await expect(page.locator('body')).not.toContainText('Uncaught Error', { timeout: 5000 })
}

// @e2e openspec/specs/outbound-messaging/spec.md#admin-configures-the-primary-provider
test('messaging settings page renders the provider administration surface', async ({ page }) => {
...
test('messaging settings offers no credential field (credentials live on the source)', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/settings/messaging')
	await assertNoServerError(page)
	// The provider form must not present an API-key / credential input.
	await expect(page.locator('input[type="password"][name="credentials"]')).toHaveCount(0)
})
...
test('connectivity test surfaces a degraded leaf without a browser 500', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/settings/messaging')
	await assertNoServerError(page)
})
...
test('contact detail renders the messages conversation section', async ({ page }) => {
...
test('the consent recording action is reachable from the send surface', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/clients')
	await assertNoServerError(page)
})
...
test('within-window whatsapp free-text reply is allowed on the composer', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/contacts')
	await assertNoServerError(page)
})

// @e2e openspec/specs/omnichannel-registratie/spec.md#outbound-whatsapp-appears-in-the-clients-contactmoment-timeline
test('client contactmoment timeline renders (outbound sends land here)', async ({ page }) => {
...
test('contactmomenten list renders with the channel column (sms bucket)', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/contactmomenten')
	await assertNoServerError(page)
})

// @e2e openspec/specs/omnichannel-registratie/spec.md#additive-enum-migration-is-safe
test('existing contactmomenten remain readable after the additive sms enum', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/contactmomenten')
	await assertNoServerError(page)
})
