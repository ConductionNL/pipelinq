...
test('request detail renders the conversion surface without a server error', async ({ page }) => {
...
test('convert-to-case action is absent when no app implements ns#Case', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/requests')
	await assertNoServerError(page)
	// On a bare instance (no ns#Case implementer) the action must not be rendered.
	await expect(page.getByRole('button', { name: /convert to case/i })).toHaveCount(0)
})
...
test('a converted request renders its read-only converted notice', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/requests')
	await assertNoServerError(page)
})

// @e2e openspec/specs/contract-renewal-tracking/spec.md#active-contract-handed-to-the-invoice-implementer
test('contract surface renders the send-to-invoicing action area', async ({ page }) => {
...
test('send-to-invoicing action is absent when no app implements ns#Invoice', async ({ page }) => {
	await page.goto('/apps/pipelinq/#/contracts')
	await assertNoServerError(page)
	await expect(page.getByRole('button', { name: /send to invoicing/i })).toHaveCount(0)
})
