...
test('Billing categories: the widget renders on the Operational dashboard', async ({ page }) => {
	await openApp(page)
	await page.goto('/apps/pipelinq/#/operational')
	await page.reload()

	const content = page.locator('#content-vue')
	await expect(content.getByText('Hours by billing category').first()).toBeVisible({ timeout: 15000 })
	await assertNoHardError(page)
})

// @e2e openspec/specs/pos-transaction-core/spec.md#billing-categories-create
test('Billing categories: the retired nav entry is gone from the sidebar', async ({ page }) => {
	await openApp(page)
	...
	await expect(
		page.locator('#app-navigation-vue').getByText('Billing categories', { exact: true }),
	).toHaveCount(0)
	await expect(
		page.locator('#app-navigation-vue').getByText('Reports & Compliance', { exact: true }),
	).toHaveCount(0)
})

// @e2e openspec/specs/pos-transaction-core/spec.md#billing-categories-list
test('Billing categories: the widget sits alongside the other operational widgets', async ({ page }) => {
	await openApp(page)
	await page.goto('/apps/pipelinq/#/operational')
	await page.reload()
