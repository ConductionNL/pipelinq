...
async function seededPolicies(page: Page): Promise<any[]> {
	...
	expect(res.status, `the seeded sla register must be readable: ${res.text}`).toBe(200)
...
function expectAttainmentEnvelope(payload: any): void {
	for (const key of ['attainment', 'attainmentPercent', 'total', 'met', 'breached',
		'inFlightBreached', 'closedBreached']) {
		expect(typeof payload?.[key], `attainment envelope key "${key}" must be numeric`).toBe('number')
	}
	...
	expect(Array.isArray(payload.details.byGroup), 'byGroup is a list of rows').toBe(true)
}
...
test.describe('SLA attainment reporting', () => {
	...
	test('a quarter-bucket report answers the documented envelope for one policy', async ({ page }) => {
		await openApp(page)

		const policies = await seededPolicies(page)
		const baseline = policies.find((p: any) => p.name === 'Standaard request-SLA')
		expect(baseline, 'the seeded baseline policy must be importable from register.d/55-sla-engine.json')
			.toBeTruthy()
		...
		expect(res.json.range?.start, 'Q2 starts on 1 April').toMatch(/^2026-04-01T00:00:00/)
		expect(res.json.range?.end, 'Q2 ends exclusive on 1 July').toMatch(/^2026-07-01T00:00:00/)
		...
		expect(typeof byTarget, 'byTarget is a per-target-kind container').toBe('object')
		for (const [kind, counts] of Object.entries<any>(byTarget ?? {})) {
			expect(['acknowledgement', 'firstResponse', 'resolution', 'callback'],
				`byTarget key "${kind}" is not a declared target kind`).toContain(kind)
			...
			expect(counts.attainment, `byTarget.${kind}.attainment must be met/(met+breached)`)
		...
		const bad = await api(page, 'GET', `${APP}/api/sla/attainment?bucket=fortnight`)
	...
	test('the report breaks down by every declared dimension and refuses unknown ones', async ({ page }) => {
		await openApp(page)
		...
		for (const [groupBy, bucket] of [['tier', 'month'], ['team', 'week'],
			['policy', 'month'], ['target', 'month'], ['customer', 'month']]) {
			const res = await api(page, 'GET', `${APP}/api/sla/attainment?groupBy=${groupBy}&bucket=${bucket}`)
			...
			for (const row of res.json.details.byGroup) {
				...
				for (const column of ['groupKey', 'groupName', 'attainment', 'total', 'met', 'breached']) {
					expect(row, `a byGroup row is missing "${column}"`).toHaveProperty(column)
				}
				// "groups with zero objects in the period MUST be omitted" — an
				// emitted row always stands for at least one counted object.
				expect(row.total, `byGroup row "${row.groupKey}" was emitted with a zero total`)
					.toBeGreaterThan(0)
				expect(row.met + row.breached, `byGroup row "${row.groupKey}" does not add up`)
					.toBeLessThanOrEqual(row.total)
			}
		}

		const bad = await api(page, 'GET', `${APP}/api/sla/attainment?groupBy=byFavouriteColour`)
		expect(bad.status, bad.text).toBe(400)
	...
	test('breached objects are reported in two separate buckets, in-flight and closed', async ({ page }) => {
		await openApp(page)

		const res = await api(page, 'GET', `${APP}/api/sla/attainment?bucket=month&groupBy=policy`)
		expect(res.status, res.text).toBe(200)
		expectAttainmentEnvelope(res.json)
		...
		expect(res.json.inFlightBreached + res.json.closedBreached,
	 ...
	 */
	// @e2e openspec/specs/sla-engine-and-escalation/spec.md#quarterly-attainment-with-breach-breakdown
	test('the SLA attainment dashboard page mounts its KPI surface', async ({ page }) => {
		await openApp(page)
...
test('a policy write without a justification is refused and persists nothing', async ({ page }) => {
	await openApp(page)
	...
	for (const expected of ['Standaard request-SLA', 'Goud-tier klant-SLA',
		'AVG datalek-klacht SLA', 'Standaard callback-SLA']) {
		expect(names, `seeded SLA policy "${expected}" is missing`).toContain(expected)
	}
	...
	const created = await api(page, 'POST', `${APP}/api/sla/policies`, {
		...
		appliesTo: 'request',
		customerTier: 'bronze',
		targets: [{ kind: 'resolution', duration: 'P1W', calendar: 'business-hours' }],
	})
	...
	expect(after.map((p: any) => p.name), 'the rejected policy was persisted anyway')
		.not.toContain(REJECTED_NAME)
	expect(after.length, 'the rejected create must not have added a policy').toBe(before.length)

	const baselineAfter = after.find((p: any) => p.name === 'Standaard request-SLA')
	const resolution = (baselineAfter?.targets ?? []).find((t: any) => t.kind === 'resolution')
	expect(resolution?.duration, 'the rejected edit must not have tightened the seeded target')
		.toBe('P3W')
})
