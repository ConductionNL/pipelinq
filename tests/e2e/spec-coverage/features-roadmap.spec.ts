...
test('Features & roadmap: navigates from sidebar and shows the features surface', async ({ page }) => {
	const errs = trackPipelinqErrors(page)
	await openApp(page)
	await navClick(page, 'Features & roadmap', /\/features-roadmap/)

	await expect(page.locator('#content-vue').getByRole('heading', { name: 'Features' }).first()).toBeVisible()
...
test('Features & roadmap: exposes roadmap + suggest-feature actions', async ({ page }) => {
	await openApp(page)
	await navClick(page, 'Features & roadmap', /\/features-roadmap/)

	const content = page.locator('#content-vue')
	await expect(content.getByRole('button', { name: 'Show roadmap' })).toBeVisible()
	await expect(content.getByRole('button', { name: 'Suggest feature' }).first()).toBeVisible()
})

// @e2e openspec/specs/notifications-activity/spec.md#features-roadmap-roadmap-toggle
test('Features & roadmap: Show roadmap reveals roadmap content', async ({ page }) => {
	await openApp(page)
	await navClick(page, 'Features & roadmap', /\/features-roadmap/)
	await dismissSupportDialog(page)

	await page.locator('#content-vue').getByRole('button', { name: 'Show roadmap' }).click()
