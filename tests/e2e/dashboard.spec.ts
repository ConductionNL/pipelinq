...
test.describe.skip('Dashboard', () => {
	...
	test('shows KPI cards with links to filtered views', async ({ page }) => {
		await expect(
			page.getByRole('heading', { name: 'Open Leads', level: 4 }),
		).toBeVisible()
		await expect(
			page.getByRole('heading', { name: 'Open Requests', level: 4 }),
		).toBeVisible()
		await expect(
			page.getByRole('heading', { name: 'Pipeline Value', level: 4 }),
		).toBeVisible()
		await expect(
			page.getByRole('heading', { name: 'Overdue', level: 4 }),
		).toBeVisible()

		await expect(page.getByRole('link', { name: /Open Leads/ })).toHaveAttribute(
			'href',
			/leads\?status=open/,
		)
		await expect(
			page.getByRole('link', { name: /Open Requests/ }),
		).toHaveAttribute('href', /requests\?status=open/)
		await expect(
			page.getByRole('link', { name: /Pipeline Value/ }),
	...
	test('shows dashboard sections', async ({ page }) => {
		await expect(
			page.getByRole('heading', { name: 'Requests by Status', level: 3 }),
		).toBeVisible()
		await expect(
			page.getByRole('heading', { name: 'My Work', level: 3 }),
		).toBeVisible()
		await expect(
			page.getByRole('heading', { name: 'Client Overview', level: 3 }),
		).toBeVisible()
	})
})
