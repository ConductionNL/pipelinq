#!/usr/bin/env bash
#
# Pipelinq API-contract test runner (Newman / Postman).
#
# Runs tests/integration/pipelinq.postman_collection.json against a live
# Nextcloud instance serving the pipelinq app. The collection is self-contained
...
LOCK_FILE="/tmp/uiaudit-pipelinq.lock"
if [ "${PIPELINQ_NEWMAN_LOCKED:-}" != "1" ] && command -v flock >/dev/null 2>&1; then
  export PIPELINQ_NEWMAN_LOCKED=1
  exec flock "${LOCK_FILE}" "$0" "$@"
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COLLECTION="${SCRIPT_DIR}/pipelinq.postman_collection.json"
...
# --ignore-redirects: assert NC's JSON 401 on unauthenticated requests directly
