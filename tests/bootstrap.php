	...
	if ($pipelinqNcRoot !== false && file_exists($pipelinqNcRoot . '/lib/base.php') === true) {
		if (pipelinq_nc_root_is_installed($pipelinqNcRoot) === true) {
			try {
				...
				// so falling through to "vendor stubs only" would be a lie that
				// costs gigabytes. Stop the run and say why.
				fwrite(
					STDERR,
					sprintf(
						"[pipelinq/tests/bootstrap] Nextcloud root at %s could not be initialised (%s).\n"
